...
- **Correctness fix.** The resampling and padding kernels were launched with
  ...
  time-series length is not a multiple of 384 and 512 — e.g. the default
  `-P 1.0` with 2^22 samples — the last few hundred samples of the resampled
  series were never written and kept the previous template's FFT output,
  which produced spurious low-frequency "candidates". All grids are now
  rounded up and bounds-checked; the mean reduction handles partial blocks
  as well. Results were verified against the upstream CPU application on
  synthetic strong- and weak-signal data: every one of the 100 reported
  candidates agrees in frequency, template and harmonic count, powers differ
  only at the 1e-6 level (cuFFT vs. FFTW rounding).
- **Faster per-template pipeline** (same numerical results):
  - the modulated-length search — a single-thread loop that walked back one
    sample at a time (up to tens of thousands of dependent global loads for
    templates with negative time offsets at the end of the data set) — is
    now a block-parallel scan that inspects 1024 samples per step;
  - the resampled length and the mean are consumed on the device, so the
    two device→host round trips per template are gone;
  - the harmonic-summing stage allocates its device/pinned buffers and CUDA
    streams once instead of per template, uploads thresholds and reads back
    dirty pages and spectra asynchronously — two synchronization points per
    template instead of eight blocking calls plus `cuMemAlloc`/`cuMemFree`;
...
- **Leaner repository.** Upstream OpenCL, Metal and CPU-only backends,
  ...
  their own path instead of hard-coded directories.
...
| `src/win_arm64/` | Symbol definitions (`.def`, generated by `scripts/wa_imports.sh`) used to build the Windows-on-ARM import libraries |
...
| `packaging/` | `app_info.xml` / `app_config.xml` variants and the per-archive README texts |

(The heavy third-party trees are intentionally not checked in — the scripts
download and patch them reproducibly. Only the CUDA backend is kept in this
repository; the upstream OpenCL / Metal / CPU-only variants and their
makefiles were removed in v1.3.)
