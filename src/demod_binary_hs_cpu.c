  ...
  for (i = 0; i < 5; i++) {
    ...
    if (dirty[i] == NULL) {
      logMessage(error, true,
                 "Couldn't allocate %d bytes of memory for sumspec page flag at bottom level.\n",
...
int run_harmonic_summing(float **sumspec,
  ...
  result = harmonic_summing(sumspec, dirty_flags, powerspectrum, window_2, fundamental_idx_hi,
                            harmonic_idx_hi, thresholds);
  return result;
}
