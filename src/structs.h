...
// portable struct packing: GCC/Clang use __attribute__, MSVC (nvcc PTX pass) needs #pragma pack
#ifdef _MSC_VER
#pragma pack(push, 1)
#define ERP_PACKED_ATTR
#else
#define ERP_PACKED_ATTR __attribute__((__packed__))
#endif
...
struct data_header {
  ...
  uint32_t filesize;     // filesize in bytes -> refers to the file generated by filterbank, not the
                         // file in which this header sits
  uint32_t datasize;     // datasize in bytes -> refers to the file generated by filterbank, not the
                         // file in which this header sits
  uint32_t nsamples;     // number of samples
  uint16_t smprec;       // Number of samples/record
  uint16_t nchan;        // Number of channels/record
  uint16_t nifs;         // Nifs = number of IF channels -> for PALFA channel 1 contains sum of two
                         // polarizations
...
struct dd_header {
  ...
  double scale;      // scale factor for compressed data. If compressed data value = 1, uncompressed
                     // value is 1/scale
  uint32_t filesize;   // filesize in bytes -> refers to the file generated by filterbank, not the
                       // file in which this header sits
  uint32_t datasize;   // datasize in bytes -> refers to the file generated by filterbank, not the
  ...
  uint16_t smprec;     // Number of samples/record
  uint16_t nchan;      // Number of channels/record
  uint16_t nifs;       // Nifs = number of polarizations -> for PALFA channel 1 contains sum of two
                       // polarizations
  uint16_t lagformat;  // lagformat
  uint16_t sum;        // sum
  uint16_t level;      // level
