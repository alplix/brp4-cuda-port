...
int boinc_get_cuda_device_id(int argc, const char **argv, int *deviceId)
  ...
  for (int cudCurrentDevice = 0; cudCurrentDevice < cudDeviceCount; ++cudCurrentDevice) {
    ...
    if (cuResult != CUDA_SUCCESS) {
      logMessage(warn, true,
                 "Couldn't retrieve multi processor count property of device #%i (error: %i)! "
                 "Trying next one...\n",
                 cudCurrentDevice, cuResult);
      continue;
    }
    ...
    // clock rate
    cuResult =
        cuDeviceGetAttribute(&clockRate, CU_DEVICE_ATTRIBUTE_CLOCK_RATE, cudCurrentDevicePtr);
    if (cuResult != CUDA_SUCCESS) {
      logMessage(
          warn, true,
    ...
    // compute capability
    cuResult = cuDeviceGetAttribute(&compcapMajor, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR,
                                    cudCurrentDevicePtr);
    if (cuResult != CUDA_SUCCESS) {
      logMessage(
          warn, true,
    ...
    cuResult = cuDeviceGetAttribute(&compcapMinor, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR,
                                    cudCurrentDevicePtr);
    if (cuResult != CUDA_SUCCESS) {
      logMessage(
          warn, true,
    ...
    if (cuResult != CUDA_SUCCESS) {
      logMessage(warn, true,
                 "Couldn't retrieve total global memory property of device #%i (error: %i)!\n",
                 cudCurrentDevice, cuResult);
    }

    // compute mode
    cuResult =
        cuDeviceGetAttribute(&computeMode, CU_DEVICE_ATTRIBUTE_COMPUTE_MODE, cudCurrentDevicePtr);
    if (cuResult != CUDA_SUCCESS) {
    ...
    if (cudGFlops > cudMaxGFlops) {
      ...
      if (!(compcapMajor >= minMajorRevision && compcapMinor >= minMinorRevision)) {
        logMessage(debug, false,
                   "Device #%i (%s) doesn't meet requested capabilities! Found: %i.%i\n",
      ...
      // CU_COMPUTEMODE_EXCLUSIVE (thread) is deprecated/removed; the modern
      // driver only offers EXCLUSIVE_PROCESS, which also blocks sharing
      if (needExclusive != 0 && computeMode == CU_COMPUTEMODE_EXCLUSIVE_PROCESS) {
...
void printDeviceGlobalMemStatus(const ERP_LOGLEVEL logLevel, const bool followUp) {
  ...
  if (cuResult == CUDA_SUCCESS) {
    ...
    logMessage(logLevel, !followUp,
               "Used in total: %u MB (%u MB free / %u MB total) -> Used by this application "
               "(assuming a single GPU task): %u MB\n",
...
CUresult loadPtxModule(CUmodule* module,
                       const char* externalFilename,
                       const char* embeddedImage,
                       const size_t embeddedImageSize) {
  // prefer an external module file if one is around (allows replacing the
  // PTX without rebuilding the application)
  if (externalFilename != NULL) {
    char path[1024] = {0};
    if (resolveFilename(externalFilename, path, sizeof(path) - 1) == 0) {
      CUresult result = cuModuleLoad(module, path);
      if (result == CUDA_SUCCESS) {
        return (CUDA_SUCCESS);
      }
      logMessage(info, true,
                 "Couldn't load external PTX module \"%s\" (error: %d), "
                 "falling back to embedded image\n",
                 path, result);
    }
  }

  // cuModuleLoadData expects a NUL-terminated PTX image
  char* image = (char*)malloc(embeddedImageSize + 1);
  if (image == NULL) {
    logMessage(error, true, "Couldn't allocate memory for embedded PTX image!\n");
    return (CUDA_ERROR_OUT_OF_MEMORY);
  }
  memcpy(image, embeddedImage, embeddedImageSize);
  image[embeddedImageSize] = '\0';

  CUresult result = cuModuleLoadData(module, image);
  free(image);
  if (result != CUDA_SUCCESS) {
    logMessage(error, true, "Couldn't load embedded PTX module (error: %d)!\n", result);
  }
  return (result);
}
