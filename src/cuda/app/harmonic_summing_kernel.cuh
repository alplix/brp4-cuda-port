...
__device__ float thrA[5];    /* threshold for 1st , 2nd, 4th, 8th, 16th harmonics   */
...
/* even if set, you can get the old functionality (e.g. for screensaver display code compatibility)
 * by setting threshold to 0.0 */

/* main kernel for harmonic summing.
 * Constraint: Blocksize must be a multiple of 16 (each sub-block of 16 consecutive threads will be
 * independent)
 *
 * Kernel consists of two logical steps:
 *
 * Step 1 : each thread looks at the values powerspectrum[i*k/16+0.5], for some thread specific i
 * and for all k=1..15 and computes sums corresponding to the harmonics. These harmonic sum spectrum
 * candidates are written to shared memory. These are candidates only because only the maximum value
 * of all candidates for any given sumspec slot will be needed, and several threads will compute
 * candidates for the same sumspec slot.
 *
 * Step 2: after all threads have finished step 1:
 *         Each sub-block of 16 consecutive threads now has created sums in shared memory that need
 * to be filtered for maximum values to compute 12 final entries into sumspec[1][*] thru
 * sumspec[4][*] slots (done one per thread, so 4 threads idle in this step). sumspec[0]is
 * powerspectrum. This leaves 3 sumspec slots unfilled "on each side" of a sub-block because data
 * dependencies from more than one sub-block (overlap). Those missing slots are taken care of by a
 * second kernel
 *
 ...
 *
 *         Each thread selects its starting index in shared memory, the number of entries to read
 * (and max), and the target indices for sumspec by looking up values from two small integer LUTs,
 * cached via texture memory.
 */

__global__ void harmonic_summing_kernel(float* sumspec1,
                                        float* sumspec2,
                                        float* sumspec3,
                                        float* sumspec4,
                                        int* dirty,
  ...
  /* initializing an array (here: lookup table h_lut and k_lut) as automatic variables would place
   * it in local memory which is NOT cached => bad instead, use a texture for look up tables for
   * reference, given here: const int  h_lut[16] = {4,3,2,2,2,  1, 1, 1, 1 , 1, 1, 1     ,-1, 3,2,1
  ...
  // for step one, one thread is doing roughly the same as the sequential ABP2 CPU version for index
  // i */

  int i = idx_j_offset + threadIdx.x + 8;  // start index of block must always be congruent to 8
                                           // modulo 16 can still be negative for left border of
                                           // spectrum, or can exceed harmonic_idx_hi-1
  int k;
  int h = i;
  ...
  /* selecting 12 threads out of every 16-thread sub block for step 2, one for every sumspec[][]
   * slot to fill with max candidate for that slot.
   */
  ...
  if ((threadIdx.x & 15) < 12) {
    ...
    h = FETCH(d_h_lut, threadIdx.x & 15);
    k = FETCH(d_k_lut, threadIdx.x & 15) + ((threadIdx.x >> 4) << 4);

    len = 1 << h; /* how many sumspec candidates we have to inspect for maximum value */
    lend2 = len >> 1;
    offset = ((h - 1) << HS_LOG_BLOCKSIZE) +
             k; /* base index for shared memory access: start index of candidates */
    lenM1 = len - 1;
...
#pragma unroll 15
    ...
    /* if we do threshold checks and the thresholds are chosen e.g. using information from the
       toplist of previous template runs, finding new, better sumspec values will be rare and the
       whole kernel essentially becomes a read-only task, with only sporadic exceptions. */

    if ((sum > FETCH(thrA, h)) && j >= 0 && (j < fundamental_idx_hi)) {
      sumspec[h - 1][j] = sum;

      /* mark this page of the sumspec array as dirty */
      /* TODO: maybe write to shared memory buffer first, then (by thread 0) to global memory if set
         to 1, avoiding simultaneous writes to global memory from the same block */
 ...
 * independent)
 *
 * Essentially this kernel does the same as the main kernel, except that it uses another "tiling" to
 * fill the gaps in sumspec arrays left unfilled by the main kernel. Logically the processing can be
 ...
 * and for all k=1..15 and computes sums corresponding to the harmonics. These harmonic sum spectrum
 * candidates are written to shared memory. These are candidates only because only the maximum value
 * of the candidates for any given sumspec slot will be needed, and several threads will compute
 * candidates for the same sumspec slot.
 *
 * Step 2: after all threads have finished step 1:
 *         Each sub-block of 2*8 consecutive threads now has created sums in shared memory that need
 * to be filtered for maximum values to compute 2*3 final entries into sumspec[1][*] thru
 * sumspec[4][*] slots (done one per thread).
 *
 ...
 *
 *         Each thread selects its starting index in shared memory, the number of entries to read
 * (and max), and the target indices for sumspec by looking up values from one tiny integer LUT,
 * cached via texture memory.
 *
 */

__global__ void harmonic_summing_kernel_gaps(float* sumspec1,
                                             float* sumspec2,
                                             float* sumspec3,
                                             float* sumspec4,
                                             int* dirty,
  ...
  if ((threadIdx.x < 4 * (HS_BLOCKSIZE / 16)) && ((threadIdx.x & 3) != 3)) {
    /* for this kernel, the lookup values for the first lookup table h_lut can directly be computed
       from index, so no need for a lookup here in this special case
     */

    //    h= FETCH(h_lut,(threadIdx.x&3) +13);

    h = 3 - (threadIdx.x & 3);
    k = FETCH(d_k_lut, (threadIdx.x & 3) + 13) + ((threadIdx.x >> 2) << 3);
    len = 1 << h;
    lend2 = len >> 1;
