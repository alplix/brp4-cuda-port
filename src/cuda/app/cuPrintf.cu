...
__device__ volatile static char *printfBufferPtr =
    NULL;  // Current atomically-incremented non-wrapped offset
...
__device__ static char *getNextPrintfBufPtr() {
  ...
  if (!printfBufferPtr) return NULL;
  ...
  if (thread_buf_len < (CUPRINTF_MAX_LEN * 2)) return NULL;
  ...
  if (offset >= hdr.thread_buf_len) offset = CUPRINTF_MAX_LEN;
  ...
  size_t offset =
      atomicAdd((unsigned int *)&printfBufferPtr, CUPRINTF_MAX_LEN) - (size_t)globalPrintfBuffer;
...
__device__ static char *cuPrintfStrncpy(char *dest, const char *src, int n, char *end) {
  ...
  if (!dest || !src || (dest >= end)) return NULL;
...
__device__ static char *copyArg(char *ptr, const char *arg, char *end) {
  // Initialisation check
  if (!ptr || !arg) return NULL;

  // strncpy does all our work. We just terminate.
  if ((ptr = cuPrintfStrncpy(ptr, arg, CUPRINTF_MAX_LEN, end)) != NULL) *ptr = 0;
...
__device__ static char *copyArg(char *ptr, T &arg, char *end) {
  ...
  if (!ptr || ((ptr + CUPRINTF_ALIGN_SIZE) >= end)) return NULL;
...
#define CUPRINTF_ARG(argname) bufptr = copyArg(bufptr, argname, end);
...
//  partitioning.
//
static int doPrintfDisplay(
    int headings, int clear, char *bufstart, char *bufend, char *bufptr, char *endptr) {
  ...
  while (bufptr != endptr) {
    ...
    if (bufptr == bufend) bufptr = bufstart;
    ...
    else if (!outputPrintfData(printfbuf_local + hdr->fmtoffset,
                               printfbuf_local + sizeof(cuPrintfHeader)))
      break;
    printf_count++;
...
extern "C" cudaError_t cudaPrintfDisplay(void *outputFP, bool showThreadID) {
  ...
  if (magic == CUPRINTF_SM10_MAGIC) {
    ...
    while (blockptr < (printfbuf_device + printfbuf_len)) {
      ...
      if (hdr.thread_buf_len != 0) blocklen = hdr.thread_buf_len;
      ...
      if (hdr.offset > 0) {
        ...
        if (sync_printfs)
          doPrintfDisplay(showThreadID, clearOnPrint, blockptr + CUPRINTF_MAX_LEN,
                          blockptr + hdr.thread_buf_len, blockptr + hdr.offset + CUPRINTF_MAX_LEN,
                          blockptr + hdr.thread_buf_len);
        doPrintfDisplay(showThreadID, clearOnPrint, blockptr + CUPRINTF_MAX_LEN,
                        blockptr + hdr.thread_buf_len, blockptr + CUPRINTF_MAX_LEN,
                        blockptr + hdr.offset + CUPRINTF_MAX_LEN);
      }
  ...
  // For SM_11 and up, everything is a single buffer and it's simple
    ...
    if (sync_printfs)
      doPrintfDisplay(showThreadID, clearOnPrint, printfbuf_device,
                      printfbuf_device + printfbuf_len, endptr, printfbuf_device + printfbuf_len);
    doPrintfDisplay(showThreadID, clearOnPrint, printfbuf_device, printfbuf_device + printfbuf_len,
                    bufptr, endptr);
  ...
  if (sync_printfs) cudaMemset(printfbuf_device, 0, printfbuf_len);
