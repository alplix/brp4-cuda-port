...
#include <cuda.h>
// GSL is used by host code elsewhere but not by this file's own code;
// keep it away from the nvcc/MSVC PTX pass
#ifndef __CUDACC__
...
// PTX image of db.dev, embedded into the executable at link time
// (see the db_dev.o rule in Makefile.win64.cuda)
...
#define PADDED_FFT_SIZE(fftsize) \
  CUDA_FFT_BLOCKDIM_X *(unsigned int)ceil((float)fftsize / (float)CUDA_FFT_BLOCKDIM_X)
...
int initialize_cuda(int cudDeviceIdGiven, int *cudDeviceIdPtr) {
  ...
  cuResult = cuCtxCreate(&cuContext, CU_CTX_SCHED_BLOCKING_SYNC, cudDeviceId);
  if (cuResult != CUDA_SUCCESS) {
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(warn, true,
               "Couldn't retrieve multiprocessor count property of device #%i (error: %i)! Trying "
               "next one...\n",
               cudDeviceId, cuResult);
  }
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(
        warn, true,
        "Couldn't retrieve clock rate property of device #%i (error: %i)! Trying next one...\n",
        cudDeviceId, cuResult);
  }
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(
        warn, true,
        "Couldn't retrieve compute capability of device #%i (error: %i)! Trying next one...\n",
        cudDeviceId, cuResult);
  }
  cuResult =
  ...
  if (compcapMajor == 9999 || compcapMinor == 9999) {
    ...
    logMessage(info, true, "Using CUDA device #%i \"%s\" (%i CUDA cores / %.2f GFLOPS)\n",
               cudDeviceId, deviceName, coreCount,
               coreCount * clockRate * flopsPerClockTick * 1e-6);
  }
  ...
  // an external db.dev file takes precedence if present)
  cuResult = loadPtxModule(&cuModuleMain, "db.dev",
                           (const char*)_binary_db_fat_start,
  ...
  cuResult =
      cuModuleGetFunction(&kernelTimeSeriesModulation, cuModuleMain, "time_series_modulation");
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true, "Couldn't get CUDA TSM kernel handle (error: %i)!\n", cuResult);
    return (RADPUL_CUDA_LOOKUP_KERNEL);
  }

  cuResult = cuModuleGetFunction(&kernelTimeSeriesLengthModulated, cuModuleMain,
                                 "time_series_length_modulated");
  if (cuResult != CUDA_SUCCESS) {
  ...
  cuResult =
      cuModuleGetFunction(&kernelTimeSeriesResampling, cuModuleMain, "time_series_resampling");
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true, "Couldn't get CUDA TSR kernel handle (error: %i)!\n", cuResult);
    return (RADPUL_CUDA_LOOKUP_KERNEL);
  }

  cuResult = cuModuleGetFunction(&kernelTimeSeriesMeanReduction, cuModuleMain,
                                 "time_series_mean_reduction");
  if (cuResult != CUDA_SUCCESS) {
...
int set_up_resampling(DIfloatPtr input_dip,
  ...
  if (params->nsamples_unpadded % CUDA_RESAMP_REDUCTION_BLOCKDIM_X != 0) {
    logMessage(error, true,
               "The time series length %i isn't an integer multiple of the CUDA block size %i!\n",
  ...
  // allocate device memory for resampled time series (we use cufftComplex*fft_size_padded here as
  // we reuse the buffer later on for the FFT)
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true,
               "Error during CUDA host->device sin lookup table data transfer (error: %i)\n",
               cuResult);
    return (RADPUL_CUDA_MEM_COPY_HOST_DEVICE);
  }
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true,
               "Error during CUDA host->device cos lookup table data transfer (error: %d)\n",
               cuResult);
    return (RADPUL_CUDA_MEM_COPY_HOST_DEVICE);
  }
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true,
               "Error during CUDA host->device lookup table parameter transfer (error: %d)\n",
               cuResult);
    return (RADPUL_CUDA_MEM_COPY_HOST_DEVICE);
  }
  ...
  cuResult =
      cuModuleGetGlobal(&timeSeriesLengthDeviceBuffer, NULL, cuModuleMain, "timeSeriesLength");
  if (cuResult != CUDA_SUCCESS) {
...
int run_resampling(DIfloatPtr input_dip, DIfloatPtr output_dip, const RESAMP_PARAMS *const params) {
  ...
  logMessage(debug, true,
             "Executing time series modulation CUDA kernel (%u threads each in %u blocks)...\n",
  ...
  void *kernelArgsTSM[] = {&modTimeOffsetsDeviceBuffer, &tau, &omega, &psi0, &dt, &step_inv, &S0};

  // launch kernel grid (n_unpadded/CUDA_RESAMP_OFFSETS_BLOCKDIM_X blocks in 1D grid)
  cuResult =
      cuLaunchKernel(kernelTimeSeriesModulation, dimGridResampOffsets.x, dimGridResampOffsets.y,
                     dimGridResampOffsets.z, dimBlockResampOffsets.x, dimBlockResampOffsets.y,
                     dimBlockResampOffsets.z, 0, NULL, kernelArgsTSM, NULL);
  if (cuResult != CUDA_SUCCESS) {
  ...
  logMessage(
      debug, true,
      "Executing modulated time series length CUDA kernel (%u threads each in %u blocks)...\n",
      dimBlockResampLength.x, dimGridResampLength.x);

  // prepare parameters
  unsigned int nsamplesUnpaddedUi = params->nsamples_unpadded;

  void *kernelArgsTSLM[] = {&modTimeOffsetsDeviceBuffer, &nsamplesUnpaddedUi};

  // launch kernel grid (one single block)
  cuResult = cuLaunchKernel(kernelTimeSeriesLengthModulated, dimGridResampLength.x,
                            dimGridResampLength.y, dimGridResampLength.z, dimBlockResampLength.x,
                            dimBlockResampLength.y, dimBlockResampLength.z, 0, NULL,
                            kernelArgsTSLM, NULL);
  if (cuResult != CUDA_SUCCESS) {
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true,
               "Error during CUDA device->host time series length transfer (error: %d)\n",
               cuResult);
  ...
  dim3 dimGridResamp(params->nsamples /
                     dimBlockResamp.x);  // nsamples/CUDA_RESAMP_BLOCKDIM_X blocks in grid (1D)

  logMessage(debug, true,
             "Executing time series resampling CUDA kernel (%u threads each in %u blocks)...\n",
             dimBlockResamp.x, dimGridResamp.x);

  // prepare parameters
  int nsamplesUnpaddedI = (int)params->nsamples_unpadded;
  int length = n_steps;

  void *kernelArgsTSR[] = {&originalTimeSeriesDeviceBuffer, &modTimeOffsetsDeviceBuffer,
                           &resampledTimeSeriesDeviceBuffer, &timeSeriesMeanDeviceBuffer,
                           &nsamplesUnpaddedI,               &length};

  // launch kernel grid (params->nsamples_unpadded / CUDA_RESAMP_BLOCKDIM_X blocks in 1D grid)
  cuResult = cuLaunchKernel(kernelTimeSeriesResampling, dimGridResamp.x, dimGridResamp.y,
                            dimGridResamp.z, dimBlockResamp.x, dimBlockResamp.y, dimBlockResamp.z,
                            0, NULL, kernelArgsTSR, NULL);
  if (cuResult != CUDA_SUCCESS) {
  ...
  int threadsPerBlockX =
      CUDA_RESAMP_REDUCTION_BLOCKDIM_X;  // CUDA_RESAMP_REDUCTION_BLOCKDIM_X threads per block (1D)
  ...
  do {
    ...
    logMessage(debug, true,
               "Executing time series mean reduction CUDA kernel (iteration %i using %i blocks of "
               "%u threads)...\n",
    ...
    if (i == 1) {
      ...
      currentInputBuffer = (CUdeviceptr)((float *)timeSeriesMeanDeviceBuffer +
                                         (useSecondHalfForOutput ? 0 : secondHalfOffset));
    }

    // alternate between first and second half of mean device buffer for output (inversely to input
    // buffer)
    currentOutputBuffer = (CUdeviceptr)((float *)timeSeriesMeanDeviceBuffer +
                                        (useSecondHalfForOutput ? secondHalfOffset : 0));

    // prepare parameters
    void *kernelArgsTSMR[] = {&currentInputBuffer, &currentOutputBuffer};

    // launch kernel grid (iteratively reduced number of threads)
    cuResult = cuLaunchKernel(kernelTimeSeriesMeanReduction, requiredBlocks, 1, 1,
                              threadsPerBlockX, 1, 1, 0, NULL, kernelArgsTSMR, NULL);
    if (cuResult != CUDA_SUCCESS) {
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true,
               "Error during CUDA device->host time series mean transfer (error: %i)\n", cuResult);
    return (RADPUL_CUDA_MEM_COPY_DEVICE_HOST);
  }
  ...
  dim3 dimBlockResampPadding(
      CUDA_RESAMP_PADDING_BLOCKDIM_X);  // CUDA_RESAMP_PADDING_BLOCKDIM_X threads per block (1D)
  dim3 dimGridResampPadding(
      params->nsamples /
      dimBlockResampPadding.x);  // nsamples/CUDA_RESAMP_BLOCKDIM_X blocks in grid (1D)

  logMessage(debug, true,
             "Executing time series padding CUDA kernel (%u threads each in %u blocks)...\n",
             dimBlockResampPadding.x, dimGridResampPadding.x);

  // prepare parameters
  int offset = n_steps;

  void *kernelArgsTSP[] = {&resampledTimeSeriesDeviceBuffer, &mean, &offset};

  // launch kernel grid (params->nsamples / CUDA_RESAMP_PADDING_BLOCKDIM_X blocks in 1D grid)
  cuResult =
      cuLaunchKernel(kernelTimeSeriesPadding, dimGridResampPadding.x, dimGridResampPadding.y,
                     dimGridResampPadding.z, dimBlockResampPadding.x, dimBlockResampPadding.y,
                     dimBlockResampPadding.z, 0, NULL, kernelArgsTSP, NULL);
  if (cuResult != CUDA_SUCCESS) {
...
int tear_down_resampling(DIfloatPtr output) {
  ...
  if (cuResult != CUDA_SUCCESS) {
    logMessage(error, true,
               "Error deallocating time series mean reduction device memory (error: %i)\n",
               cuResult);
    return (RADPUL_CUDA_MEM_FREE_DEVICE);
  }
...
int run_fft(DIfloatPtr input,
  ...
  dim3 dimGridFFT(fft_size_padded /
                  dimBlockFFT.x);  // (fft_size_padded/CUDA_FFT_BLOCKDIM_X) blocks in grid (1D)

  logMessage(debug, true,
             "Executing power spectrum CUDA kernel (%u threads each in %u blocks)...\n",
  ...
  // launch kernel grid
  cuResult = cuLaunchKernel(kernelPowerSpectrum, dimGridFFT.x, dimGridFFT.y, dimGridFFT.z,
                            dimBlockFFT.x, dimBlockFFT.y, dimBlockFFT.z, 0, NULL, kernelArgs, NULL);
  if (cuResult != CUDA_SUCCESS) {
