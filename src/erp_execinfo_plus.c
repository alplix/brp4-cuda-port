...
static int slurp_symtab(bfd *abfd) {
  ...
  if ((bfd_get_file_flags(abfd) & HAS_SYMS) == 0) return 0;
...
static int process_file(const char *file_name, const char *address) {
  ...
  if (get_file_size(file_name) < 1) return -1;
...
int backtrace_symbols_fd_plus(const char *const *symbols, int size, int fd) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (firstP != NULL) {
      ...
      fprintf(ostream, "Error: couldn't parse symbol information for file name: %s", symbols[i]);
      retval = 1;
      break;
    }
    ...
    if (firstB != NULL && secondB != NULL) {
      ...
      fprintf(ostream, "Error: couldn't parse symbol information for address: %s", symbols[i]);
      retval = 1;
      break;
    }
...
void bfd_fatal(const char *string) { bfd_nonfatal(string); }
...
off_t get_file_size(const char *file_name) {
  ...
  if (stat(file_name, &statbuf) < 0) {
    if (errno == ENOENT)
      ...
      non_fatal(_("Warning: could not locate '%s'.  reason: %s"), file_name, strerror(errno));
  }
...
int set_default_bfd_target(void) {
  ...
  if (!bfd_set_default_target(target)) {
    fatal(_("can't set BFD default target to `%s': %s"), target, bfd_errmsg(bfd_get_error()));
    return -1;
  }
