...
int harmonic_summing(float** sumspec,
                     ...
                     float* thr) {
  // loop over frequency bins and maximize the summed power at each bin for each number of summed
  // harmonics Semantics of "dirty" array: if element at the ith index of powerspectrum of 2^k th
  // harmonic is exceeding the threshold thr[k], mark the surrounding "page" of (2^LOG_PS_PAGE_SIZE)
  // elements as "dirty" by setting dirty[k][i >> LOG_PS_PAGE_SIZE] The rationale is that only
  // blocks of (2^LOG_PS_PAGE_SIZE) elements indicated by correctponding entries in dirty[][] need
  // to be inspected in the signal toplist candidate selection phase which is following after the
  // harmonic summing
  ...
  for (i = window_2; i < harmonic_idx_hi; i++) {
    ...
    // first harmonic
    sum = powerspectrum[i];
    if ((sum > thr[0]) && (i < fundamental_idx_hi)) {
