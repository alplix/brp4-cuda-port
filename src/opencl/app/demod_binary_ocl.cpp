...
// macro for fft padding (based on powerspectrum kernel's workgroup size, defined in
// demod_binary_ocl_kernels.h)
#define PADDED_FFT_SIZE(fftsize) \
  workGroupSizePS[0] * (unsigned int)ceil((float)fftsize / (float)workGroupSizePS[0])
...
size_t workGroupSizeHSG[] = {OCL_HS_WGSIZE_X / 2,
                             1};  // work group size MUST be half than that of the first kernel
...
int initialize_ocl(int oclDeviceIdGiven,
                   int *oclDeviceIdPtr,
                   cl_platform_id boincPlatformId,
                   cl_device_id boincDeviceId) {
  cl_int oclResult;
  ...
  if (boincPlatformId && boincDeviceId) {
    ...
    oclResult = clGetPlatformInfo(oclPlatform, CL_PLATFORM_VENDOR, sizeof(platformVendor),
                                  platformVendor, NULL);
    ...
    if (0 < oclNumPlatforms) {
      ...
      for (cl_uint i = 0; i < oclNumPlatforms; ++i) {
        ...
        oclResult =
            clGetPlatformInfo(platforms[i], CL_PLATFORM_PROFILE, sizeof(profile), profile, NULL);
        if (CL_SUCCESS != oclResult) {
          logMessage(
              warn, true,
              "Couldn't retrieve OpenCL platform %u/%u details (error: %i)! Trying next one...\n",
        ...
        if (!strcmp(profile, "FULL_PROFILE")) {
          ...
          logMessage(debug, true,
                     "OpenCL platform %u/%u doesn't support FULL_PROFILE (profile: %s)! Trying "
                     "next one...\n",
    ...
    if (oclDeviceIdGiven == 0) {
      ...
      logMessage(debug, true,
                 "No (valid) OpenCL device ID passed via command line. Determining suitable GPU "
                 "device... \n");
      ...
      oclResult = clGetDeviceIDs(oclPlatform, CL_DEVICE_TYPE_GPU, OCL_MAX_DEVICES, oclDevices,
                                 &oclDeviceCount);
      if (CL_SUCCESS != oclResult) {
  ...
  oclResult =
      clGetDeviceInfo(oclDeviceId, CL_DEVICE_VENDOR, sizeof(deviceVendor), deviceVendor, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult =
      clGetDeviceInfo(oclDeviceId, CL_DEVICE_NAME, sizeof(deviceProduct), deviceProduct, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clGetDeviceInfo(oclDeviceId, CL_DEVICE_COMPILER_AVAILABLE, sizeof(cl_bool),
  ...
  oclResult =
      clGetDeviceInfo(oclDeviceId, CL_DEVICE_AVAILABLE, sizeof(cl_bool), &deviceAvailable, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  // check work group size constraints for each kernel with explicit work group size (limit size if
  // necessary)
  bool needRebuild = false;
  size_t workGroupSizeLimit = 0;

  checkKernelWorkGroupInfo(&oclDeviceId, &oclKernelTimeSeriesMeanReduction, workGroupSizeTSMR[0],
                           &workGroupSizeLimit);
  ...
  checkKernelWorkGroupInfo(&oclDeviceId, &oclKernelPowerSpectrum, workGroupSizePS[0],
                           &workGroupSizeLimit);
  if (workGroupSizeLimit) {
    needRebuild = true;
  ...
  checkKernelWorkGroupInfo(&oclDeviceId, &oclKernelPowerSpectrum_radix3_r2c,
  ...
  checkKernelWorkGroupInfo(&oclDeviceId, &oclKernelHarmonicSumming, workGroupSizeHS[0],
                           &workGroupSizeLimit);
  if (workGroupSizeLimit) {
    needRebuild = true;
    ...
    if (0.0 != modf(log2(workGroupSizeLimit), &dummy)) {
      ...
      logMessage(warn, true,
                 "Work group size for \"kernelHarmonicSumming\" must be an integer power of 2! "
                 "Reducing from %d to %d...\n",
                 workGroupSizeLimit, workGroupSizeHS[0]);
    }

    workGroupSizeHSG[0] =
        workGroupSizeHS[0] / 2;  // work group size MUST be half than that of the first kernel
  }
...
int set_up_resampling(DIfloatPtr input_dip,
  ...
  if (params->nsamples_unpadded % workGroupSizeTSMR[0] != 0) {
    logMessage(
        error, true,
        "The time series length %i isn't an integer multiple of the OpenCL work group size %i!\n",
  ...
  if (len != 1 && len != 3) {
    logMessage(error, true,
               "The padded time series length %i isn't supported. Must be k*2^n, k in {1,3}!\n",
               params->nsamples);
    return (RADPUL_EVAL);
  }

  // allocate device memory for original time series
  originalTimeSeriesDeviceBuffer = clCreateBuffer(
      oclContext, CL_MEM_READ_ONLY, sizeof(float) * params->nsamples_unpadded, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating original time series device memory: %i bytes (error: %i)\n",
  ...
  sinLUTDeviceBuffer = clCreateBuffer(oclContext, CL_MEM_READ_ONLY,
                                      ERP_SINCOS_LUT_SIZE * sizeof(float), NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating sin lookup table device memory: %i bytes (error: %i)\n",
  ...
  cosLUTDeviceBuffer = clCreateBuffer(oclContext, CL_MEM_READ_ONLY,
                                      ERP_SINCOS_LUT_SIZE * sizeof(float), NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating cos lookup table device memory: %i bytes (error: %i)\n",
               ERP_SINCOS_LUT_SIZE * sizeof(float), oclResult);
    return (RADPUL_OCL_MEM_ALLOC_DEVICE);
  }
  ...
  modTimeOffsetsDeviceBuffer = clCreateBuffer(
      oclContext, CL_MEM_READ_WRITE, sizeof(float) * params->nsamples_unpadded, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating modulated time offsets device memory: %i bytes (error: %i)\n",
               sizeof(float) * params->nsamples_unpadded, oclResult);
    return (RADPUL_OCL_MEM_ALLOC_DEVICE);
  }
  ...
  timeSeriesLengthDeviceBuffer =
      clCreateBuffer(oclContext, CL_MEM_READ_WRITE, sizeof(int), NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(
        error, true,
        "Error allocating modulated time series length device memory: %i bytes (error: %i)\n",
  ...
  // allocate device memory for resampled time series (we need twice the amount of samples as buffer
  // because of the fake C2C FFT input (split-complex)
  resampledTimeSeriesDeviceBuffer = clCreateBuffer(
      oclContext, CL_MEM_READ_WRITE, 2 * sizeof(float) * params->nsamples, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating modulated time series device memory: %i bytes (error: %d)\n",
  ...
  // allocate device memory for time series mean sum reduction (we need two separate buffers, each
  // half used alternately)
  timeSeriesMeanDeviceBuffer = clCreateBuffer(
      oclContext, CL_MEM_READ_WRITE,
      sizeof(float) * (params->nsamples_unpadded / workGroupSizeTSMR[0] * 2), NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
  ...
  fftComplexDeviceBuffer = clCreateBuffer(oclContext, CL_MEM_READ_WRITE,
                                          sizeof(float) * 2 * complexLen, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clEnqueueWriteBuffer(oclQueue, originalTimeSeriesDeviceBuffer, CL_FALSE, 0,
                                   sizeof(float) * params->nsamples_unpadded, input, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL host->device original time series data transfer (error: %i)\n",
               oclResult);
  ...
  oclResult =
      clEnqueueWriteBuffer(oclQueue, sinLUTDeviceBuffer, CL_FALSE, 0,
                           ERP_SINCOS_LUT_SIZE * sizeof(float), sinLUTsamples, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL host->device sin lookup table data transfer (error: %i)\n",
               oclResult);
    return (RADPUL_OCL_MEM_COPY_HOST_DEVICE);
  }
  ...
  oclResult =
      clEnqueueWriteBuffer(oclQueue, cosLUTDeviceBuffer, CL_FALSE, 0,
                           ERP_SINCOS_LUT_SIZE * sizeof(float), cosLUTsamples, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL host->device cos lookup table data transfer (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_MEM_COPY_HOST_DEVICE);
  }
...
int run_resampling(DIfloatPtr input_dip, DIfloatPtr output_dip, const RESAMP_PARAMS *const params) {
  ...
  // prepare parameters
  oclResult = clSetKernelArg(oclKernelTimeSeriesModulation, 0, sizeof(sinLUTDeviceBuffer),
                             &sinLUTDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clSetKernelArg(oclKernelTimeSeriesModulation, 1, sizeof(cosLUTDeviceBuffer),
                             &cosLUTDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: TSM (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelTimeSeriesModulation, 3, sizeof(params->Omega), &params->Omega);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: TSM (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  oclResult =
      clSetKernelArg(oclKernelTimeSeriesModulation, 6, sizeof(params->step_inv), &params->step_inv);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: TSM (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  oclResult = clSetKernelArg(oclKernelTimeSeriesModulation, 8, sizeof(modTimeOffsetsDeviceBuffer),
  ...
  // invoke kernel grid
  oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelTimeSeriesModulation, 1, NULL,
                                     workItemAmountTSM, NULL, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  // determine modulated time series length (done in kernel because global sync required and to
  // avoid bus transfer of del_t as well as waste of host memory)

  logMessage(debug, true,
             "Executing modulated time series length OpenCL kernel (single work item)...\n");

  // prepare parameters
  oclResult = clSetKernelArg(oclKernelTimeSeriesLengthModulated, 0,
                             sizeof(params->nsamples_unpadded), &params->nsamples_unpadded);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clSetKernelArg(oclKernelTimeSeriesLengthModulated, 1,
                             sizeof(modTimeOffsetsDeviceBuffer), &modTimeOffsetsDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: TSLM (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  oclResult = clSetKernelArg(oclKernelTimeSeriesLengthModulated, 2,
                             sizeof(timeSeriesLengthDeviceBuffer), &timeSeriesLengthDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: TSLM (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  oclResult = clEnqueueReadBuffer(oclQueue, timeSeriesLengthDeviceBuffer, CL_FALSE, 0,
                                  sizeof(n_steps), &n_steps, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL device->host time series length transfer (error: %d)\n",
               oclResult);
  ...
  // prepare parameters
  oclResult =
      clSetKernelArg(oclKernelTimeSeriesResampling, 0, sizeof(originalTimeSeriesDeviceBuffer),
  ...
  oclResult = clSetKernelArg(oclKernelTimeSeriesResampling, 1, sizeof(modTimeOffsetsDeviceBuffer),
                             &modTimeOffsetsDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: TSR (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelTimeSeriesResampling, 3, sizeof(resampledTimeSeriesDeviceBuffer),
  ...
  // launch kernel grid
  oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelTimeSeriesResampling, 1, NULL,
                                     workItemAmountTSR, NULL, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  do {
    // use kernel decomposition to do sum reduction (facilitates global memory sync, kernel
    // invocations are cheap)

    const size_t workItemAmountTSMR[] = {requiredBlocks * currentWorkGroupSizeTSMR[0]};
    logMessage(debug, true,
               "Executing time series mean reduction OpenCL kernel (iteration %i using %i work "
               "groups of %u work items)...\n",
    ...
    if (1 == i) {
      ...
      // otherwise: alternate between first and second half of mean device buffer for input
      // (inversely to output buffer)
      offsetInput = useSecondHalfForOutput ? 0 : secondHalfOffset;
      currentInputBuffer = timeSeriesMeanDeviceBuffer;
    }

    // alternate between first and second half of mean device buffer for output (inversely to input
    // buffer)
    ...
    oclResult = clSetKernelArg(oclKernelTimeSeriesMeanReduction, 0, sizeof(currentInputBuffer),
                               &currentInputBuffer);
    if (CL_SUCCESS != oclResult) {
    ...
    oclResult = clSetKernelArg(oclKernelTimeSeriesMeanReduction, 1, sizeof(currentOutputBuffer),
                               &currentOutputBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: TSMR-%i (error: %d)\n",
                 i, oclResult);
    ...
    oclResult =
        clSetKernelArg(oclKernelTimeSeriesMeanReduction, 2, sizeof(offsetInput), &offsetInput);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: TSMR-%i (error: %d)\n",
                 i, oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    oclResult =
        clSetKernelArg(oclKernelTimeSeriesMeanReduction, 3, sizeof(offsetOutput), &offsetOutput);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: TSMR-%i (error: %d)\n",
                 i, oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelTimeSeriesMeanReduction, 1, NULL,
                                       workItemAmountTSMR, currentWorkGroupSizeTSMR, 0, NULL, NULL);
    if (CL_SUCCESS != oclResult) {
    ...
    if (requiredBlocks >= currentWorkGroupSizeTSMR[0]) {
      ...
      if (requiredBlocks == 1) {
        ...
        // we're now within the last block (with fewer than blocksize elements), so sum pairs with
        // one thread each
  ...
  oclResult =
      clEnqueueReadBuffer(oclQueue, currentOutputBuffer, CL_TRUE, offsetOutput * sizeof(float),
                          sizeof(float), &mean, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL device->host time series sum transfer (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_MEM_COPY_DEVICE_HOST);
  }
  ...
  //    in this case the output is produced in the second half of the input array and the first half
  //    is zeroed out in prep. for FFT step
  ...
  if (fftradix == 1) {
    ...
    oclResult =
        clSetKernelArg(oclKernelTimeSeriesPadding, 2, sizeof(resampledTimeSeriesDeviceBuffer),
                       &resampledTimeSeriesDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: TSMP (error: %d)\n",
                 oclResult);
    ...
    oclResult = clSetKernelArg(oclKernelTimeSeriesPadding, 3, sizeof(fftComplexDeviceBuffer),
                               &fftComplexDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: TSMP (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    // launch kernel grid
    oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelTimeSeriesPadding, 1, NULL,
                                       workItemAmountTSMP, NULL, 0, NULL, NULL);
    if (CL_SUCCESS != oclResult) {
    ...
    logMessage(debug, true,
               "Executing time series padding & transpose OpenCL kernel %lu times...\n",
               workItemAmountTSMP[0]);
    ...
    oclResult =
        clSetKernelArg(oclKernelTimeSeriesPaddingTranspose, 3, sizeof(subFFTlen), &subFFTlen);
    if (CL_SUCCESS != oclResult) {
    ...
    oclResult =
        clSetKernelArg(oclKernelTimeSeriesPaddingTranspose, 4,
                       sizeof(resampledTimeSeriesDeviceBuffer), &resampledTimeSeriesDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
    ...
    oclResult = clSetKernelArg(oclKernelTimeSeriesPaddingTranspose, 5,
                               sizeof(fftComplexDeviceBuffer), &fftComplexDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: TSMP_T5 (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    // launch kernel grid
    oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelTimeSeriesPaddingTranspose, 1, NULL,
                                       workItemAmountTSMP, NULL, 0, NULL, NULL);
    if (CL_SUCCESS != oclResult) {
...
int set_up_fft(DIfloatPtr input_dip,
  ...
  // create FFT plan
  clFftPlan = clFFT_CreatePlanAdv(oclContext, sampleSpace, clFFT_1D, clFFT_InterleavedComplexFormat,
                                  clFFT_TaylorLUT, &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  powerSpectrumDeviceBuffer = clCreateBuffer(oclContext, CL_MEM_READ_WRITE,
                                             sizeof(float) * fft_size_padded, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  // we are using interleaved format to pack a length N Real to Complex transform into a length N/2
  // Complex 2 Complex transform The "untangling" to get the right results for the Real 2 Complex
  // transform is done in the kernel that also performs the computation of the power and the radix 3
  // butterflies in case of N=3*2^n

  // pre-compute twiddle factors for radix-k butterfly
  // TODO: two alternatives: optimized per k or generic. let's start with k=3
  // TODO: it's somewhat ugly that we pass the plan as global var, if we could make it generic
  // across
  // TODO: FFT impls we could pass it via function argument. then also the twiddle factors might be
  // stored
  ...
  fftTwiddleFactorDeviceBuffer = clCreateBuffer(oclContext, CL_MEM_READ_ONLY,
                                                2 * sizeof(float) * nsamples / 2, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  fftTwiddleFactor_r2cDeviceBuffer = clCreateBuffer(
      oclContext, CL_MEM_READ_ONLY, 2 * sizeof(float) * nsamples / 2, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating twiddle factor (2) device memory: %i bytes (error: %i)\n",
               sizeof(float) * 2 * nsamples / 2, oclResult);
    return (RADPUL_OCL_MEM_ALLOC_DEVICE);
  }
  ...
  oclResult = clEnqueueWriteBuffer(oclQueue, fftTwiddleFactorDeviceBuffer, CL_TRUE, 0,
                                   2 * nsamples / 2 * sizeof(float), twiddle_f, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL host->device fft twiddle factors data transfer (error: %i)\n",
               oclResult);
    return (RADPUL_OCL_MEM_COPY_HOST_DEVICE);
  }
  logMessage(debug, true, "OpenCL host->device fft twiddle factors data transfer ...\n");

  // Precompute second set of twiddle factors for untangling the "packed" C2C transform result to
  // the initial R2C transform result
  ...
  oclResult = clEnqueueWriteBuffer(oclQueue, fftTwiddleFactor_r2cDeviceBuffer, CL_FALSE, 0,
                                   2 * nsamples / 2 * sizeof(float), twiddle_f, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(
        error, true,
        "Error during OpenCL host->device fft twiddle factors (2) data transfer (error: %i)\n",
        oclResult);
    return (RADPUL_OCL_MEM_COPY_HOST_DEVICE);
  }
  ...
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL buffer synchronization: twiddle factors (2) (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_INVOKE);
  }
...
int run_fft(DIfloatPtr input_dip,
            DIfloatPtr output_dip,
            uint32_t nsamples,
            unsigned int fft_size,
            float norm_factor) {
  ...
  // increase number of powerspectrum threads such that all blocks/threads are used completely (no
  // further control flow required in kernel)
  ...
  oclResult =
      clFFT_ExecuteInterleaved(oclQueue, clFftPlan, fftradix, clFFT_Forward, fftComplexDeviceBuffer,
                               fftComplexDeviceBuffer, 0, NULL, NULL);
  ...
  if (fftradix == 1) {
    const size_t workItemAmountPS[] = {
        packedSubFFTlen / 2 + workGroupSizePS[0]};  // half the length of the complex(!) DFT + 1 .
                                                    // Each thread computes 2 elements of the PS

    logMessage(debug, true,
               "Executing powerspectrum OpenCL kernel (%u work items each in %u work groups)...\n",
               workGroupSizePS[0], workItemAmountPS[0] / workGroupSizePS[0]);

    // prepare parameters
    oclResult = clSetKernelArg(oclKernelPowerSpectrum, 0, sizeof(fftComplexDeviceBuffer),
                               &fftComplexDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
    ...
    oclResult = clSetKernelArg(oclKernelPowerSpectrum, 1, sizeof(fftTwiddleFactor_r2cDeviceBuffer),
                               &fftTwiddleFactor_r2cDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
    ...
    oclResult =
        clSetKernelArg(oclKernelPowerSpectrum, 3, sizeof(packedSubFFTlen), &packedSubFFTlen);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    oclResult = clSetKernelArg(oclKernelPowerSpectrum, 4, sizeof(powerSpectrumDeviceBuffer),
    ...
    // launch kernel grid
    oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelPowerSpectrum, 1, NULL, workItemAmountPS,
                                       workGroupSizePS, 0, NULL, NULL);
    if (CL_SUCCESS != oclResult) {
  ...
  if (fftradix == 3) {
    ...
    logMessage(debug, true,
               "Executing powerspectrum (radix 3) OpenCL kernel (%u work items each in %u work "
               "groups)...\n",
               workGroupSizePS_R3_R2C[0], workItemAmountPS_R3_R2C[0] / workGroupSizePS_R3_R2C[0]);

    // prepare parameters
    oclResult = clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 0, sizeof(fftComplexDeviceBuffer),
                               &fftComplexDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
    ...
    oclResult = clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 1,
                               sizeof(fftTwiddleFactorDeviceBuffer), &fftTwiddleFactorDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    oclResult =
        clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 2,
                       sizeof(fftTwiddleFactor_r2cDeviceBuffer), &fftTwiddleFactor_r2cDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    }
    oclResult = clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 3, sizeof(packedSubFFTlen),
                               &packedSubFFTlen);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
    ...
    oclResult = clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 4, sizeof(fft_size_padded),
                               &fft_size_padded);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
    ...
    oclResult = clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 5,
                               sizeof(butterfly_twiddle_radix3), &butterfly_twiddle_radix3);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    oclResult =
        clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 6, sizeof(norm_factor), &norm_factor);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    oclResult = clSetKernelArg(oclKernelPowerSpectrum_radix3_r2c, 7,
                               sizeof(powerSpectrumDeviceBuffer), &powerSpectrumDeviceBuffer);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true, "Error during OpenCL kernel parameter setup: PS_R3 (error: %d)\n",
                 oclResult);
      return (RADPUL_OCL_KERNEL_SETUP);
    ...
    // launch kernel grid
    oclResult =
        clEnqueueNDRangeKernel(oclQueue, oclKernelPowerSpectrum_radix3_r2c, 1, NULL,
                               workItemAmountPS_R3_R2C, workGroupSizePS_R3_R2C, 0, NULL, NULL);
    if (CL_SUCCESS != oclResult) {
...
int set_up_harmonic_summing(float **sumspec,
  ...
  for (i = 1; i < 5; ++i) {
    sumspecDev[i] = clCreateBuffer(oclContext, CL_MEM_WRITE_ONLY,
                                   sizeof(float) * fundamental_idx_hi, NULL, &oclResult);
    if (CL_SUCCESS != oclResult) {
      logMessage(error, true,
                 "Error allocating harmonic summing spectra device memory: %i bytes (error: %i)\n",
  ...
  *nr_pages_ptr = nr_pages;
  ...
  harmomicSumminghLUTDeviceBuffer =
      clCreateBuffer(oclContext, CL_MEM_READ_ONLY, sizeof(int32_t) * 16, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating harmonic summing h-LUT device memory: %i bytes (error: %i)\n",
  ...
  harmomicSummingkLUTDeviceBuffer =
      clCreateBuffer(oclContext, CL_MEM_READ_ONLY, sizeof(int32_t) * 16, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating harmonic summing k-LUT device memory: %i bytes (error: %i)\n",
               sizeof(int32_t) * 16, oclResult);
    return (RADPUL_OCL_MEM_ALLOC_DEVICE);
  }
  ...
  harmomicSummingThresholdsDeviceBuffer =
      clCreateBuffer(oclContext, CL_MEM_READ_ONLY, sizeof(float) * 5, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error allocating harmonic summing thresholds device memory: %i bytes (error: %i)\n",
  ...
  oclResult = clEnqueueWriteBuffer(oclQueue, harmomicSumminghLUTDeviceBuffer, CL_FALSE, 0,
                                   sizeof(int32_t) * 16, h_lut, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clEnqueueWriteBuffer(oclQueue, harmomicSummingkLUTDeviceBuffer, CL_FALSE, 0,
                                   sizeof(int32_t) * 16, k_lut, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
...
int run_harmonic_summing(float **sumspec,
  ...
  for (i = 1; i < 5; ++i) {
    ...
    // prepare parameters
    oclResult = clSetKernelArg(oclKernelFillFloatBuffer, 0, sizeof(fundamental_idx_hi),
                               &fundamental_idx_hi);
    if (CL_SUCCESS != oclResult) {
    ...
    // launch kernel grid
    oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelFillFloatBuffer, 1, NULL,
                                       workItemAmountFFB, NULL, 0, NULL, NULL);
    if (CL_SUCCESS != oclResult) {
  ...
  dirtyDeviceBuffer = clCreateBuffer(oclContext, CL_MEM_WRITE_ONLY,
                                     sizeof(int32_t) * nr_pages_total, NULL, &oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelFillIntBuffer, 2, sizeof(dirtyDeviceBuffer), &dirtyDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HSFIB (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  // launch kernel grid
  oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelFillIntBuffer, 1, NULL, workItemAmountFIB,
                                     NULL, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clEnqueueWriteBuffer(oclQueue, harmomicSummingThresholdsDeviceBuffer, CL_FALSE, 0,
                                   sizeof(float) * 5, thresholds, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true,
               "Error during OpenCL host->device HS thresholds data transfer (error: %i)\n",
               oclResult);
    return (RADPUL_OCL_MEM_COPY_HOST_DEVICE);
  }
  ...
  logMessage(debug, true,
             "Executing harmonic summing OpenCL kernel (%u work items each in %u work groups)...\n",
             workGroupSizeHS[0], (workItemAmountHS[0] * workItemAmountHS[1]) / workGroupSizeHS[0]);

  // prepare parameters
  for (i = 1; i < 5; ++i) {
    oclResult =
        clSetKernelArg(oclKernelHarmonicSumming, i - 1, sizeof(sumspecDev[i]), &sumspecDev[i]);
    if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clSetKernelArg(oclKernelHarmonicSumming, 4, sizeof(powerSpectrumDeviceBuffer),
                             &powerSpectrumDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSumming, 5, sizeof(dirtyDeviceBuffer), &dirtyDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  oclResult = clSetKernelArg(oclKernelHarmonicSumming, 6, sizeof(harmomicSumminghLUTDeviceBuffer),
                             &harmomicSumminghLUTDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
  ...
  oclResult = clSetKernelArg(oclKernelHarmonicSumming, 7, sizeof(harmomicSummingkLUTDeviceBuffer),
                             &harmomicSummingkLUTDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSumming, 8, sizeof(harmomicSummingThresholdsDeviceBuffer),
                     &harmomicSummingThresholdsDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSumming, 10, sizeof(fundamental_idx_hi), &fundamental_idx_hi);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSumming, 11, sizeof(harmonic_idx_hi), &harmonic_idx_hi);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  // launch kernel grid
  oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelHarmonicSumming, 2, NULL, workItemAmountHS,
                                     workGroupSizeHS, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  for (i = 1; i < 5; ++i) {
    oclResult =
        clSetKernelArg(oclKernelHarmonicSummingGaps, i - 1, sizeof(sumspecDev[i]), &sumspecDev[i]);
    if (CL_SUCCESS != oclResult) {
  ...
  logMessage(
      debug, true,
      "Executing harmonic summing gaps OpenCL kernel (%u work items each in %u work groups)...\n",
      workGroupSizeHSG[0], (workItemAmountHSG[0] * workItemAmountHSG[1]) / workGroupSizeHSG[0]);

  oclResult = clSetKernelArg(oclKernelHarmonicSummingGaps, 4, sizeof(powerSpectrumDeviceBuffer),
                             &powerSpectrumDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HSG (error: %d)\n",
               oclResult);
  ...
  oclResult = clSetKernelArg(oclKernelHarmonicSummingGaps, 5, sizeof(dirtyDeviceBuffer),
                             &dirtyDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HS (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSummingGaps, 6, sizeof(harmomicSumminghLUTDeviceBuffer),
                     &harmomicSumminghLUTDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HSG (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSummingGaps, 7, sizeof(harmomicSummingkLUTDeviceBuffer),
                     &harmomicSummingkLUTDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HSG (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSummingGaps, 8, sizeof(harmomicSummingThresholdsDeviceBuffer),
                     &harmomicSummingThresholdsDeviceBuffer);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HSG (error: %d)\n",
               oclResult);
  ...
  oclResult = clSetKernelArg(oclKernelHarmonicSummingGaps, 10, sizeof(fundamental_idx_hi),
                             &fundamental_idx_hi);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HSG (error: %d)\n",
               oclResult);
  ...
  oclResult =
      clSetKernelArg(oclKernelHarmonicSummingGaps, 11, sizeof(harmonic_idx_hi), &harmonic_idx_hi);
  if (CL_SUCCESS != oclResult) {
    logMessage(error, true, "Error during OpenCL kernel parameter setup: HSG (error: %d)\n",
               oclResult);
    return (RADPUL_OCL_KERNEL_SETUP);
  ...
  // launch kernel grid
  oclResult = clEnqueueNDRangeKernel(oclQueue, oclKernelHarmonicSummingGaps, 2, NULL,
                                     workItemAmountHSG, workGroupSizeHSG, 0, NULL, NULL);
  if (CL_SUCCESS != oclResult) {
  ...
  oclResult = clEnqueueReadBuffer(oclQueue, dirtyDeviceBuffer, CL_TRUE, 0,
                                  sizeof(int32_t) * nr_pages_total, dirtyTmp, 0, NULL, NULL);
  ...
  for (i = 0; i < 5; i++) {
    ...
    if (dirty_idx_max[i] != -1) {
      ...
      oclResult =
          clEnqueueReadBuffer(oclQueue, sumspecDev[i], CL_TRUE, sizeof(float) * seg_offset,
                              sizeof(float) * seg_length, sumspec[i] + seg_offset, 0, NULL, NULL);
      if (CL_SUCCESS != oclResult) {
        logMessage(error, true,
                   "Error during OpenCL device->host powerspectrum transfer (error: %d)\n",
                   oclResult);
        return (RADPUL_OCL_MEM_COPY_DEVICE_HOST);
      }
    }
...
cl_int createProgramKernels(const cl_device_id oclDeviceId, const cl_uint oclPlatformVendor) {
  ...
  int result = snprintf(compileOptions, maxLength, defaultCompileOptions, vendorOptions,
                        workGroupSizeTSMR[0], workGroupSizePS[0], workGroupSizePS_R3_R2C[0],
                        workGroupSizeHS[0], lrint(log2(workGroupSizeHS[0])), LOG_PS_PAGE_SIZE);
  ...
  oclProgramDemodBinary =
      clCreateProgramWithSource(oclContext, 13, programSources, NULL, &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  clGetProgramBuildInfo(oclProgramDemodBinary, oclDeviceId, CL_PROGRAM_BUILD_LOG, NULL, NULL,
                        &buildLogLength);
  if (1 < buildLogLength) {
    char *buildLog = (char *)calloc(buildLogLength, sizeof(char));
    if (buildLog) {
      clGetProgramBuildInfo(oclProgramDemodBinary, oclDeviceId, CL_PROGRAM_BUILD_LOG,
                            buildLogLength * sizeof(char), buildLog, NULL);
  ...
  // prepare kernels
  oclKernelTimeSeriesModulation =
      clCreateKernel(oclProgramDemodBinary, "kernelTimeSeriesModulation", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelTimeSeriesLengthModulated =
      clCreateKernel(oclProgramDemodBinary, "kernelTimeSeriesLengthModulated", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelTimeSeriesResampling =
      clCreateKernel(oclProgramDemodBinary, "kernelTimeSeriesResampling", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelTimeSeriesMeanReduction =
      clCreateKernel(oclProgramDemodBinary, "kernelTimeSeriesMeanReduction", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelTimeSeriesPadding =
      clCreateKernel(oclProgramDemodBinary, "kernelTimeSeriesPadding", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelTimeSeriesPaddingTranspose =
      clCreateKernel(oclProgramDemodBinary, "kernelTimeSeriesPaddingTranspose", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelPowerSpectrum_radix3_r2c =
      clCreateKernel(oclProgramDemodBinary, "kernelPowerSpectrum_radix3_r2c", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelHarmonicSumming =
      clCreateKernel(oclProgramDemodBinary, "kernelHarmonicSumming", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelHarmonicSummingGaps =
      clCreateKernel(oclProgramDemodBinary, "kernelHarmonicSummingGaps", &oclResult);
  if (CL_SUCCESS != oclResult) {
  ...
  oclKernelFillFloatBuffer =
      clCreateKernel(oclProgramDemodBinary, "kernelFillFloatBuffer", &oclResult);
  if (CL_SUCCESS != oclResult) {
