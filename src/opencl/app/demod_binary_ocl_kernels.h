...
static const char kernelTimeSeriesLengthModulated[] =
    ...
    "    /* nearest_idx (see time_series_resampling kernel) must not exceed n_unpadded - 1, so go "
    "back as far as needed to ensure that */ \n"
...
static const char kernelTimeSeriesResampling[] =
    ...
    ""
    "        /* set i-th bin in resampled time series (at the pulsar) to nearest_idx bin from "
...
static const char kernelPowerSpectrum[] =
    "#define complexMul(a,b) ((float2)(-((a).y)*(b).y + (a).x * (b).x, (a).y* (b).x + (a).x * "
    "(b).y))\n"
    "\n"
...
static const char kernelPowerSpectrum_radix3_r2c[] =
    ...
    "    localTwiddle_mod2[localId].y = "
    "fft_twiddle_data[globalId+subFFTlength+subFFTlength+fullFFTlength]; \n"
    ""
    ...
    "    localTwiddle_modM1[localId].y = "
    "fft_twiddle_data[index_reverse+subFFTlength+fullFFTlength]; \n"
    "    localTwiddle_modM2[localId].x = "
    "fft_twiddle_data[index_reverse+subFFTlength+subFFTlength]; \n"
    "    localTwiddle_modM2[localId].y = "
    "fft_twiddle_data[index_reverse+subFFTlength+subFFTlength+fullFFTlength]; \n"
    ""
    "    /* wait for load to finish */ \n"
    "    barrier(CLK_LOCAL_MEM_FENCE); \n"
    ""
    ...
    ""
    "    localFFTData_mod1[localId] = "
    "complexMul(localFFTData_mod1[localId],localTwiddle_mod1[localId])  ;\n"
    "    localFFTData_modM1[localId] =  "
    "complexMul(localFFTData_modM1[localId],localTwiddle_modM1[localId]);\n"
    ""
    ""
    "    localFFTData_mod2[localId] = "
    "complexMul(localFFTData_mod2[localId],localTwiddle_mod2[localId])  ;\n"
    "    localFFTData_modM2[localId] = "
    "complexMul(localFFTData_modM2[localId],localTwiddle_modM2[localId]);\n"
    "                                                                \n"
    "                                                                \n"
    "    // butterfly step                                           \n"
    ...
    "    localTwiddle_mod2[localId].y = "
    "fft_twiddle_data_rc[globalId+subFFTlength+subFFTlength+fullFFTlength]; \n"
    ""
    "    /* wait for load to finish */ \n"
    "    barrier(CLK_LOCAL_MEM_FENCE); \n"
    ""
    ...
    "\n"
    "    localTwiddle_modM1[localId].y = "
    "fft_twiddle_data_rc[subFFTlength+subFFTlength-globalId+fullFFTlength]; \n"
    ""
    "    /* wait for load to finish */ \n"
    "    barrier(CLK_LOCAL_MEM_FENCE); \n"
    ""
    ...
    "    localTwiddle_modM2[localId].x = "
    "fft_twiddle_data_rc[(subFFTlength+subFFTlength+subFFTlength-globalId) % fullFFTlength]; \n"
    "    localTwiddle_modM2[localId].y = "
    "fft_twiddle_data_rc[((subFFTlength+subFFTlength+subFFTlength-globalId) % fullFFTlength "
    ")+fullFFTlength]; \n"
    ""
    ...
    "    if(!ignore) ps_data[globalId+subFFTlength] = norm_factor * (res.x * res.x + res.y * "
    "res.y); \n"
    ""
    "    tf= localTwiddle_mod2[localId];\n"
    "    untangle(res,r2_1,r0_2,tf);  \n"
    "    if(!ignore) ps_data[globalId+subFFTlength+subFFTlength] = norm_factor * (res.x * res.x + "
    "res.y * res.y); \n"
    ""
    ...
    "    if(!ignore) ps_data[subFFTlength-globalId] = norm_factor * (res.x * res.x + res.y * "
    "res.y); \n"
    "    tf= localTwiddle_modM1[localId]; \n"
    "    untangle(res,r1_2,r1_1,tf);  \n"
    "    if(!ignore) ps_data[subFFTlength+subFFTlength-globalId] = norm_factor * (res.x * res.x + "
    "res.y * res.y); \n"
    "    tf= localTwiddle_modM2[localId]; \n"
    "    untangle(res,r2_2,r0_1,tf);  \n"
    "    if(!ignore) ps_data[subFFTlength+subFFTlength+subFFTlength-globalId] = norm_factor * "
...
static const char kernelHarmonicSumming[] =
    ...
    "    int idx_j_offset =  (idx_j << OCL_HS_LOG_WGSIZE_X) + -16 ; // negative index to handle "
    "left border \n"
    ...
    "        /* TODO: maybe write to shared memory buffer first, then (by thread 0) to global "
    "memory if set to 1, avoiding \n"
    "                simultaneous writes to global memory from the same block */                   "
    "                       \n"
    "\n"
    "            dirty[((fundamental_idx_hi >> OCL_LOG_PS_PAGE_SIZE_X) +1)  * h + (j>> "
    "OCL_LOG_PS_PAGE_SIZE_X)] = 1;                  \n"
    "            sumspec[h-1][j] = sum; \n"
    "        } \n"
    "    } \n"
    "} \n";
...
static const char kernelHarmonicSummingGaps[] =
    ...
    "    int i2, i4, i8; \n"
    "    int iN; \n"
    "    /* for this kernel , there can be now overlap with the left border of the spectrum (index "
    "0), \n"
