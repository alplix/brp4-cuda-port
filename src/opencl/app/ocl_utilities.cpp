...
cl_device_id findBestFreeDevice(const cl_platform_id platform,
  ...
  for (cl_uint i = 0; i < oclDeviceCount; ++i) {
    ...
    oclResult =
        clGetDeviceInfo(oclDevices[i], CL_DEVICE_NAME, sizeof(deviceName), deviceName, NULL);
    if (CL_SUCCESS != oclResult) {
    ...
    oclResult = clGetDeviceInfo(oclDevices[i], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(compUnits),
                                &compUnits, NULL);
    if (CL_SUCCESS != oclResult) {
      logMessage(warn, true,
                 "Couldn't retrieve required OpenCL device property of device \"%s\" (error: %i)! "
                 "Trying next one...\n",
    ...
    oclResult = clGetDeviceInfo(oclDevices[i], CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(clockFreq),
                                &clockFreq, NULL);
    if (CL_SUCCESS != oclResult) {
      logMessage(warn, true,
                 "Couldn't retrieve required OpenCL device property of device \"%s\" (error: %i)! "
    ...
    if (0 < minGlobalMemoryBytes) {
      oclResult = clGetDeviceInfo(oclDevices[i], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(totalGlobalMem),
                                  &totalGlobalMem, NULL);
      if (CL_SUCCESS != oclResult) {
        logMessage(warn, true,
                   "Couldn't retrieve required OpenCL device property of device \"%s\" (error: "
                   "%i)! Trying next one...\n",
                   deviceName, oclResult);
        continue;
      }
    ...
    if (0 < minMajorVersion || 0 < minMinorVersion) {
      oclResult = clGetDeviceInfo(oclDevices[i], CL_DEVICE_VERSION, sizeof(deviceVersion),
                                  &deviceVersion, NULL);
      if (CL_SUCCESS != oclResult) {
        logMessage(warn, true,
                   "Couldn't retrieve required OpenCL device property of device \"%s\" (error: "
        ...
        if (2 != sscanf(deviceVersion, "OpenCL %i.%i", &oclDeviceMajor, &oclDeviceMinor)) {
          logMessage(warn, true,
                     "Couldn't retrieve required OpenCL device version property of device \"%s\"! "
                     "Trying next one...\n",
                     deviceName);
          continue;
        }
    ...
    oclResult = clGetDeviceInfo(oclDevices[i], CL_DEVICE_EXTENSIONS, sizeof(deviceExtensions),
                                &deviceExtensions, NULL);
    if (CL_SUCCESS != oclResult) {
      logMessage(
          warn, true,
          "Couldn't retrieve OpenCL device extensions property of device \"%s\" (error: %i)!\n",
          deviceName, oclResult);
    }
    else {
      ...
      if (string::npos != extensions.find("cl_nv_device_attribute_query")) {
        logMessage(debug, true,
                   "Evaluating extended NVIDIA properties of OpenCL device \"%s\"...\n",
                   deviceName);
        nvidia = true;
      }
    }
    ...
    if (nvidia) {
      ...
      oclResult = clGetDeviceInfo(oclDevices[i], CL_DEVICE_COMPUTE_CAPABILITY_MAJOR_NV,
                                  sizeof(nvCompCapMajor), &nvCompCapMajor, NULL);
      if (CL_SUCCESS != oclResult) {
        logMessage(warn, true,
                   "Couldn't retrieve NVIDIA OpenCL device compute capability (error: %i)!\n",
                   oclResult);
      }
      oclResult = clGetDeviceInfo(oclDevices[i], CL_DEVICE_COMPUTE_CAPABILITY_MINOR_NV,
                                  sizeof(nvCompCapMinor), &nvCompCapMinor, NULL);
      if (CL_SUCCESS != oclResult) {
        logMessage(warn, true,
                   "Couldn't retrieve NVIDIA OpenCL device compute capability (error: %i)!\n",
                   oclResult);
    ...
    if (oclPerformance > oclMaxPerformance) {
      ...
      if (oclDeviceMajor < minMajorVersion ||
          (oclDeviceMajor == minMajorVersion && oclDeviceMinor < minMinorVersion)) {
        logMessage(debug, true,
                   "Device \"%s\" doesn't meet requested OpenCL version (requested: %i.%i / found: "
                   "%i.%i)! Trying next one...\n",
      ...
      if (totalGlobalMem < minGlobalMemoryBytes) {
        logMessage(debug, true,
                   "Device \"%s\" doesn't meet global memory requirements (requested: %i bytes / "
                   "found: %i bytes)! Trying next one...\n",
  ...
  if (NULL != oclBestDevice) {
    ...
    oclResult = clGetDeviceInfo(oclBestDevice, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                                sizeof(dimensions), &dimensions, NULL);
    if (CL_SUCCESS != oclResult) {
      logMessage(warn, true,
                 "Couldn't retrieve OpenCL device number of work item dimensions (error: %i)!\n",
                 oclResult);
    }
    else {
      ...
      if (NULL == itemsPerDimension) {
        logMessage(warn, true,
                   "Couldn't allocate %d bytes of host memory for OpenCL device work item "
                   "dimensions sizes!\n",
                   dimensions * sizeof(size_t));
      }
      else {
        oclResult = clGetDeviceInfo(oclBestDevice, CL_DEVICE_MAX_WORK_ITEM_SIZES,
                                    dimensions * sizeof(size_t), itemsPerDimension, NULL);
        if (CL_SUCCESS != oclResult) {
          logMessage(warn, true,
                     "Couldn't retrieve OpenCL device work item dimensions sizes (error: %i)!\n",
                     oclResult);
        }
        else {
...
cl_int dumpFloatDeviceBufferToTextFile(const cl_command_queue queue,
  ...
  oclResult = clEnqueueReadBuffer(queue, buffer, CL_TRUE, 0, sizeof(float) * size, hostBuffer, 0,
                                  NULL, NULL);
  if (CL_SUCCESS != oclResult) {
...
cl_int checkKernelWorkGroupInfo(const cl_device_id *devicePtr,
  ...
  oclResult = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_WORK_GROUP_SIZE,
  ...
  if (maxWorkGroupSize < workGroupSize) {
    logMessage(warn, true,
               "Kernel \"%s\" exceeds device-specific maximum work group size (requested: %u)!\n",
               kernelName, workGroupSize);
    logMessage(warn, false,
               "Reducing kernel's work group size to allowed maximum of: %u work items\n",
               maxWorkGroupSize);
  ...
  oclResult = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE,
  ...
  oclResult = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_LOCAL_MEM_SIZE,
  ...
  oclResult = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_PRIVATE_MEM_SIZE,
...
void contextErrorCallback(const char *errinfo,
