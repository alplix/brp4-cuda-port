...
// libxml2 must be initialized exactly once before any API call; static
// builds with thread support crash otherwise (uninitialized global tables)
static bool erp_xml_initialized = false;

static void erp_xml_init_once(void) {
  if (!erp_xml_initialized) {
    xmlInitParser();
    erp_xml_initialized = true;
  }
}
...
void erp_update_shmem(void) {
  ...
  try {
    ...
    xmlNewChild(rootNode, NULL, BAD_CAST("power_spectrum"),
                BAD_CAST(powerSpectrumString.str().data()));
    ...
    xmlNewChild(boincStatusNode, NULL, BAD_CAST("reread_init_data_file"),
                BAD_CAST(converter.str().c_str()));
    converter.str("");

    converter << dec << boinc_status.abort_request;
    xmlNewChild(boincStatusNode, NULL, BAD_CAST("abort_request"),
                BAD_CAST(converter.str().c_str()));
    converter.str("");
    ...
    xmlNewChild(boincStatusNode, NULL, BAD_CAST("working_set_size"),
                BAD_CAST(converter.str().c_str()));
    converter.str("");

    converter << boinc_status.max_working_set_size;
    xmlNewChild(boincStatusNode, NULL, BAD_CAST("max_working_set_size"),
                BAD_CAST(converter.str().c_str()));
  } catch (ios_base::failure) {
...
void erp_boinc_init(void) { boinc_init_options(&erp_boinc_options); }
