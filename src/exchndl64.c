...
// The GetModuleBase function retrieves the base address of the module that contains the specified
// address.
static SIZE_T GetModuleBase(DWORD64 dwAddress) {
  MEMORY_BASIC_INFORMATION Buffer;

  return VirtualQuery((LPCVOID)dwAddress, &Buffer, sizeof(Buffer)) ? (SIZE_T)Buffer.AllocationBase
                                                                   : 0;
}
...
static bfd_boolean slurp_symtab(bfd *abfd, asymbol ***syms, long *symcount) {
  long storage;

  if ((bfd_get_file_flags(abfd) & HAS_SYMS) == 0) return FALSE;
  ...
  *syms = (asymbol **)LocalAlloc(LMEM_FIXED, storage);
  ...
  if ((*symcount = bfd_canonicalize_symtab(abfd, *syms)) < 0) return FALSE;

  return TRUE;
}

// This stucture is used to pass information between translate_addresses and
...
static void find_address_in_section(bfd *abfd, asection *section, PTR data) {
  ...
  if (info->pc < (vma = bfd_section_vma(section))) return;

  if (info->pc >= vma + (size = bfd_section_size(section))) return;

  info->found = bfd_find_nearest_line(abfd, section, info->syms, info->pc - vma, &info->filename,
                                      &info->functionname, &info->line);
}

static BOOL BfdDemangleSymName(LPCTSTR lpName, LPTSTR lpDemangledName, DWORD nSize) {
  char *res;
...
static BOOL BfdGetSymFromAddr(
    bfd *abfd, asymbol **syms, long symcount, DWORD64 dwAddress, LPTSTR lpSymName, DWORD nSize) {
  HMODULE hModule;
  struct find_handle info;

  if (!(hModule = (HMODULE)GetModuleBase(dwAddress))) return FALSE;

  info.pc = dwAddress;

  if (!(bfd_get_file_flags(abfd) & HAS_SYMS) || !symcount) return FALSE;
  info.syms = syms;

  info.found = FALSE;
  bfd_map_over_sections(abfd, find_address_in_section, (PTR)&info);
  if (info.found == FALSE || info.line == 0) return FALSE;

  assert(lpSymName);

  if (info.functionname == NULL && *info.functionname == '\0') return FALSE;
...
static BOOL BfdGetLineFromAddr(bfd *abfd,
                               asymbol **syms,
                               long symcount,
                               DWORD64 dwAddress,
                               LPTSTR lpFileName,
...
static BOOL WINAPI j_SymInitialize(HANDLE hProcess, PSTR UserSearchPath, BOOL fInvadeProcess) {
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymInitialize ||
       (pfnSymInitialize = (PFNSYMINITIALIZE)GetProcAddress(hModule_Imagehlp, "SymInitialize"))))
...
static BOOL WINAPI j_SymCleanup(HANDLE hProcess) {
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymCleanup ||
       (pfnSymCleanup = (PFNSYMCLEANUP)GetProcAddress(hModule_Imagehlp, "SymCleanup"))))
...
static DWORD WINAPI j_SymSetOptions(DWORD SymOptions) {
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymSetOptions ||
       (pfnSymSetOptions = (PFNSYMSETOPTIONS)GetProcAddress(hModule_Imagehlp, "SymSetOptions"))))
...
static BOOL WINAPI j_SymUnDName64(PIMAGEHLP_SYMBOL64 Symbol,
                                  PSTR UnDecName,
                                  DWORD UnDecNameLength) {
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymUnDName64 ||
       (pfnSymUnDName64 = (PFNSYMUNDNAME64)GetProcAddress(hModule_Imagehlp, "SymUnDName64"))))
...
static PVOID WINAPI j_SymFunctionTableAccess64(HANDLE hProcess, DWORD64 AddrBase) {
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymFunctionTableAccess64 ||
       (pfnSymFunctionTableAccess64 = (PFNSYMFUNCTIONTABLEACCESS64)GetProcAddress(
            hModule_Imagehlp, "SymFunctionTableAccess64"))))
...
static DWORD64 WINAPI j_SymGetModuleBase64(HANDLE hProcess, DWORD64 dwAddr) {
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymGetModuleBase64 || (pfnSymGetModuleBase64 = (PFNSYMGETMODULEBASE64)GetProcAddress(
                                     hModule_Imagehlp, "SymGetModuleBase64"))))
    return pfnSymGetModuleBase64(hProcess, dwAddr);
  else
    return 0;
}

typedef BOOL(WINAPI *PFNSTACKWALK64)(DWORD,
                                     HANDLE,
                                     HANDLE,
                                     LPSTACKFRAME64,
                                     LPVOID,
...
static BOOL WINAPI j_StackWalk64(DWORD MachineType,
                                 HANDLE hProcess,
                                 HANDLE hThread,
  ...
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnStackWalk64 ||
       (pfnStackWalk64 = (PFNSTACKWALK64)GetProcAddress(hModule_Imagehlp, "StackWalk64"))))
    return pfnStackWalk64(MachineType, hProcess, hThread, StackFrame, ContextRecord,
...
static BOOL WINAPI j_SymGetSymFromAddr64(HANDLE hProcess,
                                         DWORD64 Address,
                                         PDWORD64 Displacement,
                                         PIMAGEHLP_SYMBOL64 Symbol) {
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymGetSymFromAddr64 || (pfnSymGetSymFromAddr64 = (PFNSYMGETSYMFROMADDR64)GetProcAddress(
                                      hModule_Imagehlp, "SymGetSymFromAddr64"))))
...
static BOOL WINAPI j_SymGetLineFromAddr64(HANDLE hProcess,
                                          DWORD64 dwAddr,
  ...
  if ((hModule_Imagehlp || (hModule_Imagehlp = LoadLibrary(_T("IMAGEHLP.DLL")))) &&
      (pfnSymGetLineFromAddr64 ||
       (pfnSymGetLineFromAddr64 =
            (PFNSYMGETLINEFROMADDR64)GetProcAddress(hModule_Imagehlp, "SymGetLineFromAddr64"))))
    return pfnSymGetLineFromAddr64(hProcess, dwAddr, pdwDisplacement, Line);
  else
    return FALSE;
}

static BOOL CALLBACK ReadProcessMemoryArgumentWrapper(HANDLE hProcess,
                                                      DWORD64 lpBaseAddress,
                                                      PVOID lpBuffer,
                                                      DWORD nSize,
                                                      LPDWORD lpNumberOfBytesRead) {
  return ReadProcessMemory(hProcess, (LPCVOID)lpBaseAddress, (LPVOID)lpBuffer, (SIZE_T)nSize,
                           (SIZE_T *)lpNumberOfBytesRead);
}

static BOOL ImagehlpDemangleSymName(LPCTSTR lpName, LPTSTR lpDemangledName, DWORD nSize) {
  ...
  if (!j_SymUnDName64(pSymbol, lpDemangledName, nSize)) return FALSE;

  return TRUE;
}

static BOOL ImagehlpGetSymFromAddr(HANDLE hProcess,
                                   DWORD64 dwAddress,
                                   LPTSTR lpSymName,
                                   DWORD nSize) {
  ...
  if (!j_SymGetSymFromAddr64(hProcess, dwAddress, &dwDisplacement, pSymbol)) return FALSE;
...
static BOOL ImagehlpGetLineFromAddr(
    HANDLE hProcess, DWORD dwAddress, LPTSTR lpFileName, DWORD nSize, LPDWORD lpLineNumber) {
...
#if 1
  {
    ...
    DWORD dwTempDisp = 0;
    while (dwTempDisp < 100 &&
           !j_SymGetLineFromAddr64(hProcess, dwAddress - dwTempDisp, &dwDisplacement, &Line))
      ++dwTempDisp;

    if (dwTempDisp >= 100) return FALSE;

    // It was found and the source line information is correct so
    //  change the displacement if it was looked up multiple times.
    if (dwTempDisp < 100 && dwTempDisp != 0) dwDisplacement = dwTempDisp;
  }
#else
  if (!j_SymGetLineFromAddr(hProcess, dwAddress, &dwDisplacement, &Line)) return FALSE;
#endif
...
static BOOL PEGetSymFromAddr(HANDLE hProcess, DWORD64 dwAddress, LPTSTR lpSymName, DWORD64 nSize) {
  HMODULE hModule;
  ...
  pSection = (PIMAGE_SECTION_HEADER)(pNtHdr + sizeof(DWORD) + sizeof(IMAGE_FILE_HEADER) +
  ...
  for (i = 0; i < NtHdr.FileHeader.NumberOfSections; i++, pSection++) {
    ...
    if (memcmp(Section.Name, ExportSectionName, IMAGE_SIZEOF_SHORT_NAME) == 0)
    ...
    else if ((NtHdr.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress >=
      ...
      pExportDir = (PIMAGE_EXPORT_DIRECTORY)(DWORD64)NtHdr.OptionalHeader
    ...
    if (pExportDir) {
      ...
      if (!ReadProcessMemory(hProcess, (PVOID)((DWORD64)hModule + (DWORD64)pExportDir), &ExportDir,
...

      {
        ...
        if (!ReadProcessMemory(
                hProcess, (PVOID)((DWORD64)hModule + (DWORD64)ExportDir.AddressOfFunctions),
                AddressOfFunctions, ExportDir.NumberOfFunctions * sizeof(PDWORD), NULL))
          return FALSE;

        for (j = 0; j < ExportDir.NumberOfNames; ++j) {
          DWORD64 pFunction = (DWORD64)hModule + (DWORD64)AddressOfFunctions[j];
          // ReadProcessMemory(hProcess, (DWORD) hModule + (DWORD)
          // (&ExportDir.AddressOfFunctions[j]), &pFunction, sizeof(pFunction), NULL);

          if (pFunction <= dwAddress && pFunction > dwNearestAddress) {
            dwNearestAddress = pFunction;

            if (!ReadProcessMemory(
                    hProcess, (PVOID)((DWORD64)hModule + (DWORD64)(&ExportDir.AddressOfNames)[j]),
                    &dwNearestName, sizeof(dwNearestName), NULL))
              return FALSE;
  ...
  if (!dwNearestAddress) return FALSE;

  if (!ReadProcessMemory(hProcess, (PVOID)dwNearestName, lpSymName, nSize, NULL)) return FALSE;
  lpSymName[nSize - 1] = 0;

  return TRUE;
}

static BOOL WINAPI IntelStackWalk64(DWORD MachineType,
                                    HANDLE hProcess,
                                    HANDLE hThread,
                                    LPSTACKFRAME64 StackFrame,
                                    ...
                                    PGET_MODULE_BASE_ROUTINE64 GetModuleBaseRoutine,
                                    PTRANSLATE_ADDRESS_ROUTINE64 TranslateAddress) {
  if (ReadMemoryRoutine == NULL) ReadMemoryRoutine = &ReadProcessMemoryArgumentWrapper;
  ...
  if (!StackFrame->Reserved[0]) {
    ...
    if (!ReadMemoryRoutine(hProcess, StackFrame->AddrFrame.Offset + sizeof(DWORD64),
                           &StackFrame->AddrReturn.Offset, sizeof(DWORD64), NULL))
      return FALSE;
  }
  else {
    StackFrame->AddrPC.Offset = StackFrame->AddrReturn.Offset;
    // AddrStack = AddrFrame + 2*sizeof(DWORD);
    if (!ReadMemoryRoutine(hProcess, StackFrame->AddrFrame.Offset, &StackFrame->AddrFrame.Offset,
                           sizeof(DWORD64), NULL))
      return FALSE;
    if (!ReadMemoryRoutine(hProcess, StackFrame->AddrFrame.Offset + sizeof(DWORD64),
                           &StackFrame->AddrReturn.Offset, sizeof(DWORD64), NULL))
  ...
  ReadMemoryRoutine(hProcess, StackFrame->AddrFrame.Offset + 2 * sizeof(DWORD64),
                    StackFrame->Params, sizeof(StackFrame->Params), NULL);

  return TRUE;
}

static BOOL StackBackTrace(HANDLE hProcess, HANDLE hThread, PCONTEXT pContext) {
  ...
  if (j_SymInitialize(hProcess, NULL, TRUE)) bSymInitialized = TRUE;
  ...
  while (1) {
    ...
    if (bSymInitialized) {
      if (!j_StackWalk64(IMAGE_FILE_MACHINE_AMD64, hProcess, hThread, &StackFrame, pContext, NULL,
                         j_SymFunctionTableAccess64, j_SymGetModuleBase64, NULL))
        break;
    }
    else {
      if (!IntelStackWalk64(IMAGE_FILE_MACHINE_AMD64, hProcess, hThread, &StackFrame, pContext,
                            NULL, NULL, NULL, NULL))
        break;
    }

    // Basic sanity check to make sure  the frame is OK.  Bail if not.
    if (0 == StackFrame.AddrFrame.Offset) break;

    if (0) {
      rprintf(_T("%08lX   %08lX   %08lX   %08lX\r\n"), StackFrame.AddrPC.Offset,
              StackFrame.AddrReturn.Offset, StackFrame.AddrFrame.Offset,
    ...
    if ((hModule = (HMODULE)GetModuleBase(StackFrame.AddrPC.Offset)) &&
      ...
      if (hModule != hPrevModule) {
        ...
        if ((abfd = bfd_openr(szModule, NULL)))
          if (bfd_check_format(abfd, bfd_object)) {
            ...
            if (bfd_get_file_flags(abfd) & HAS_SYMS) /* Read in the symbol table.  */
              slurp_symtab(abfd, &syms, &symcount);
          }
      }

      if (!bSuccess && abfd && syms && symcount)
        if ((bSuccess = BfdGetSymFromAddr(abfd, syms, symcount, StackFrame.AddrPC.Offset, szSymName,
                                          512))) {
          /*
          ...
          if (BfdGetLineFromAddr(abfd, syms, symcount, StackFrame.AddrPC.Offset, szFileName,
                                 MAX_PATH, &LineNumber))
      ...
      if (!bSuccess && bSymInitialized)
        if ((bSuccess =
                 ImagehlpGetSymFromAddr(hProcess, StackFrame.AddrPC.Offset, szSymName, 512))) {
          rprintf(_T("  %s"), szSymName);

          ImagehlpDemangleSymName(szSymName, szSymName, 512);

          if (ImagehlpGetLineFromAddr(hProcess, StackFrame.AddrPC.Offset, szFileName, MAX_PATH,
                                      &LineNumber))
            rprintf(_T("  %s:%ld"), szFileName, LineNumber);
        }
  ...
  if (bSymInitialized) {
    if (!j_SymCleanup(hProcess)) assert(0);
...
static void GenerateExceptionReport(PEXCEPTION_POINTERS pExceptionInfo) {
  ...
  {
    const TCHAR *lpDayOfWeek[] = {_T("Sunday"),   _T("Monday"), _T("Tuesday"), _T("Wednesday"),
                                  _T("Thursday"), _T("Friday"), _T("Saturday")};
    const TCHAR *lpMonth[] = {NULL,          _T("January"),   _T("February"), _T("March"),
                              _T("April"),   _T("May"),       _T("June"),     _T("July"),
                              _T("August"),  _T("September"), _T("October"),  _T("November"),
                              _T("December")};
    SYSTEMTIME SystemTime;

    GetLocalTime(&SystemTime);
    rprintf(_T("Error occured on %s, %s %i, %i at %02i:%02i:%02i.\r\n\r\n"),
            lpDayOfWeek[SystemTime.wDayOfWeek], lpMonth[SystemTime.wMonth], SystemTime.wDay,
  ...
  if ((hModule = (HMODULE)GetModuleBase((DWORD64)pExceptionRecord->ExceptionAddress)) &&
      GetModuleFileName(hModule, szModule, sizeof(szModule)))
  ...
  if (pContext->ContextFlags & CONTEXT_INTEGER)
    rprintf(_T("eax=%08lx ebx=%08lx ecx=%08lx edx=%08lx esi=%08lx edi=%08lx\r\n"), pContext->Eax,
            pContext->Ebx, pContext->Ecx, pContext->Edx, pContext->Esi, pContext->Edi);
  if (pContext->ContextFlags & CONTEXT_CONTROL)
    rprintf(_T("eip=%08lx esp=%08lx ebp=%08lx iopl=%1lx %s %s %s %s %s %s %s %s %s %s\r\n"),
  ...
  if (pContext->ContextFlags & CONTEXT_SEGMENTS) {
    rprintf(_T("cs=%04lx  ss=%04lx  ds=%04lx  es=%04lx  fs=%04lx  gs=%04lx"), pContext->SegCs,
            pContext->SegSs, pContext->SegDs, pContext->SegEs, pContext->SegFs, pContext->SegGs,
            pContext->EFlags);
    if (pContext->ContextFlags & CONTEXT_CONTROL)
      rprintf(_T("             efl=%08lx"), pContext->EFlags);
  }
