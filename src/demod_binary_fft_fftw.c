...
int set_up_fft(DIfloatPtr input, DIfloatPtr *output, uint32_t nsamples, unsigned int fft_size) {
  // allocate memory for FFT (use of fftwf_malloc recommended by FFTW, see manual: section 2.1, page
  // 3)
...
int run_fft(DIfloatPtr input,
  ...
  for (unsigned int i = 1; i < fft_size; i++) {
    output.host_ptr[i] =
        norm_factor * (gsl_pow_2(output.host_ptr[i + i]) + gsl_pow_2(output.host_ptr[i + i + 1]));
  }

#else
  for (unsigned int i = 1; i < fft_size; i++) {
    output.host_ptr[i] =
        norm_factor * (gsl_pow_2(t_series_resamp_fft[i][0]) + gsl_pow_2(t_series_resamp_fft[i][1]));
  }
#endif
  // set DC power to 0
