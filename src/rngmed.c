...
void rngmed(const float *input,
  ...
  */
  ...
  if (ceil((float)midpoint / (float)stepchkpts) != (float)midpoint / (float)stepchkpts) ncheckpts++;
#endif
  ...
  /* init nodes array */
  for (i = 0; i < bsize; i++) nodes[i].value = input[i];
  for (i = 1; i < bsize - 1; i++) {
  ...
  /* setup checkpoints */
#ifndef NEWCHECKPOINT
  for (i = 0; i < ncheckpts; i++) checkpts[i] = qsnodes[i * stepchkpts].index;
#else
...
#ifndef NEWCHECKPOINT
  for (i = 0; i < mdnoffset; i++) nextnode = nodes[nextnode].greater;
#endif
  if (isodd)
  ...
  for (nmedian = 1; nmedian < length - bsize + 1; nmedian++) {
    ...
    for (rightcheckpt = 0; rightcheckpt < ncheckpts; rightcheckpt++)
      if (newvalue <= nodes[checkpts[rightcheckpt]].value) break;

    /* assume we are inserting at the beginning: */
    prevnode = nil;
    if (rightcheckpt == 0) /* yes, we are */
      nextnode = checkpts[0];
    else {
    ...
    */
    ...
    if ((oldestnode != prevnode) && (oldestnode != nextnode)) {
      ...
      if (prevnode != nil) nodes[prevnode].greater = oldestnode;
      if (nextnode != nil) nodes[nextnode].lesser = oldestnode;
