...
static const char *_getopt_initialize(int argc,
  ...
  if (optstring[0] == '-') {
    d->__ordering = getopt_data.RETURN_IN_ORDER;
    ++optstring;
  }
  else if (optstring[0] == '+') {
    d->__ordering = getopt_data.REQUIRE_ORDER;
    ++optstring;
  }
  else if (d->__posixly_correct)
    d->__ordering = getopt_data.REQUIRE_ORDER;
  else
    d->__ordering = getopt_data.PERMUTE;
  ...
  if (!d->__posixly_correct && argc == __libc_argc && argv == __libc_argv) {
    if (d->__nonoption_flags_max_len == 0) {
      if (__getopt_nonoption_flags == NULL || __getopt_nonoption_flags[0] == '\0')
        ...
        if (d->__nonoption_flags_max_len < argc) d->__nonoption_flags_max_len = argc;
...
int _getopt_internal_r(int argc,
  ...
  if (d->optind == 0 || !d->__initialized) {
    if (d->optind == 0) d->optind = 1; /* Don't scan ARGV[0], the program name.  */
...
#define NONOPTION_P (argv[d->optind][0] != '-' || argv[d->optind][1] == '\0')
  ...
  if (d->__nextchar == NULL || *d->__nextchar == '\0') {
    ...
    if (d->__ordering == getopt_data.PERMUTE) {
    ...
    if (d->optind == argc) {
      ...
      if (d->__first_nonopt != d->__last_nonopt) d->optind = d->__first_nonopt;
      return -1;
    }
    ...
    if (NONOPTION_P) {
      if (d->__ordering == getopt_data.REQUIRE_ORDER) return -1;
  ...
  if (longopts != NULL &&
    ...
    for (nameend = d->__nextchar; *nameend && *nameend != '='; nameend++) /* Do nothing.  */
      ;
