...
#define PRINT_FPU_EXCEPTION_MASK(fpstat)                                                           \
  if (fpstat & FPU_STATUS_PRECISION) logMessage(error, false, "FPU exception: PRECISION\n");       \
  if (fpstat & FPU_STATUS_UNDERFLOW) logMessage(error, false, "FPU exception: UNDERFLOW\n");       \
  if (fpstat & FPU_STATUS_OVERFLOW) logMessage(error, false, "FPU exception: OVERFLOW\n");         \
  if (fpstat & FPU_STATUS_ZERO_DIVIDE) logMessage(error, false, "FPU exception: ZERO_DIVIDE\n");   \
  if (fpstat & FPU_STATUS_DENORMALIZED) logMessage(error, false, "FPU exception: DENORMALIZED\n"); \
  if (fpstat & FPU_STATUS_INVALID) logMessage(error, false, "FPU exception: INVALID\n")
#define PRINT_FPU_STATUS_FLAGS(fpstat)                                                        \
  if (fpstat & FPU_STATUS_COND_3) logMessage(error, false, "FPU status: COND_3\n");           \
  if (fpstat & FPU_STATUS_COND_2) logMessage(error, false, "FPU status: COND_2\n");           \
  if (fpstat & FPU_STATUS_COND_1) logMessage(error, false, "FPU status: COND_1\n");           \
  if (fpstat & FPU_STATUS_COND_0) logMessage(error, false, "FPU status: COND_0\n");           \
  if (fpstat & FPU_STATUS_ERROR_SUMM) logMessage(error, false, "FPU status: ERR_SUMM\n");     \
  if (fpstat & FPU_STATUS_STACK_FAULT) logMessage(error, false, "FPU status: STACK_FAULT\n"); \
...
#define MAXFRAMES 128
  ...
  char** frames_resolved = backtrace_symbols(callstack, frames);
  logMessage(error, true, "Backtrace:\n");
  for (int i = 0; i < frames; ++i) {
    logMessage(error, false, "%s\n", frames_resolved[i]);
  }
  free(frames_resolved);
#endif
...
gzFile boinc_gzopen(const char* path, const char* mode) {
  ...
  // zlib needs to take ownership of its own file descriptor
  int fd = fileno(fp);
  gzFile gzf = gzdopen(dup(fd), mode);
  close(fd);
  return gzf;
}

void erp_fraction_done(double frac) { boinc_fraction_done((frac + pass) / passes); }
...
static int worker(void) {
  ...
  while (true) {
    ...
    static struct option long_options[] = {{"input-file", required_argument, 0, 'i'},
  ...
  for (pass = 0; pass < passes && !result; pass++) {
    ...
    if (FILE* fp = fopen(forward_argv.at(forward_argc - 1), "r")) {
      ...
      if (result) {
        ...
        if (checkpoint_file) unlink(checkpoint_file);
      }
    }
...
int main(int argc, char** argv) {
  ...
  boinc_init_diagnostics(BOINC_DIAG_DUMPCALLSTACKENABLED | BOINC_DIAG_HEAPCHECKENABLED |
                         BOINC_DIAG_ARCHIVESTDERR | BOINC_DIAG_REDIRECTSTDERR |
  ...
  boinc_set_signal_handler(SIGTERM, (handler_t)sighandler);
  boinc_set_signal_handler(SIGINT, (handler_t)sighandler);
  boinc_set_signal_handler(SIGABRT, (handler_t)sighandler);
  ...
  if (result == RADPUL_CUDA_MEM_ALLOC_HOST || result == RADPUL_CUDA_MEM_ALLOC_DEVICE ||
      result == RADPUL_CUDA_FFT_PLAN || result == RADPUL_OCL_MEM_ALLOC_HOST ||
      result == RADPUL_OCL_MEM_ALLOC_DEVICE) {
    logMessage(warn, true,
               "Sorry, at the moment your system doesn't have enough free CPU/GPU memory to run "
               "this task!\n");
    ...
    boinc_temporary_exit(900,
                         "Not enough free CPU/GPU memory available! Delaying next attempt for at "
                         "least 15 minutes...");
  }
  else {
