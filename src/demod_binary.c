...
#define NCAND_TIMES_NH uvar.Ncand *(uvar.harmonics_log2 + 1)
#define NCAND_TIMES_NH_PTR uvar->Ncand *(uvar->harmonics_log2 + 1)
...
typedef struct {
  ...
  int harmonics_log2;                      // log2 of the number of harmonics to sum
  unsigned int Ncand;                      // Number of candidates to output
} User_Variables;

typedef struct {
  float P;     // Orbital period (s)
  float tau;   // Orbital semi-major axis (lt s)
  float Psi0;  // Orbital phase (rad) at start of data set
               // Psi0 = 0 is the pulsar's descending node.
} Binary_Template;
...
int MAIN(int argc, char *argv[]) {
  ...
  Binary_Template *template_params = NULL;  // array of template parameters
  ...
  unsigned int
      fundamental_idx_hi;        // frequency bin of the highest fundamental frequency searched for
  ...
  unsigned int
      n_unpadded_format;  // modified number of samples based on input file format (4-bit/8-bit)
  ...
  static const char *c_template_scan_format = "%g %g %g\n";
  ...
  while (i < argc) {
    if ((strcmp(argv[i], "-W") == 0) || (strcmp(argv[i], "--whitening") == 0)) {
    ...
    else if ((strcmp(argv[i], "-H") == 0) || (strcmp(argv[i], "--harmonics_log2") == 0)) {
      // sanity check
      if (atoi(argv[i + 1]) < 0) {
        logMessage(error, true, "Nonsense value: harmonics %d is negative.\n", atoi(argv[i + 1]));
        return (RADPUL_EVAL);
      }
      else if (atoi(argv[i + 1]) > 4) {
        logMessage(
            error, true,
            "Nonsense value: harmonics %d > 4 (at most 2^14=16 harmonics can be searched).\n",
            atoi(argv[i + 1]));
        return (RADPUL_EVAL);
      }
      else {
        uvar.harmonics_log2 = atoi(argv[i + 1]);
        i += 2;
      }
    }
    else if ((strcmp(argv[i], "-N") == 0) || (strcmp(argv[i], "--Ncand") == 0)) {
      // sanity check
      if (atoi(argv[i + 1]) < 0) {
        logMessage(error, true, "Nonsense value: Ncand %d is negative.\n", atoi(argv[i + 1]));
        return (RADPUL_EVAL);
      }
      else {
        uvar.Ncand = atoi(argv[i + 1]);
        i += 2;
      }
    }
      ...
      if (snprintf(uvar.outputfile_tmp, sizeof(uvar.outputfile_tmp), "%s.tmp", argv[i + 1]) >=
          sizeof(uvar.outputfile_tmp)) {
      ...
      if (snprintf(uvar.checkpointfile_tmp, sizeof(uvar.checkpointfile_tmp), "%s.tmp",
                   argv[i + 1]) >= sizeof(uvar.checkpointfile_tmp)) {
  ...
  candidates_all = (CP_cand *)calloc(NCAND_TIMES_NH, sizeof(CP_cand));
  if (candidates_all == NULL) {
    logMessage(error, true, "Couldn't allocate %d bytes of memory for candidates_all.\n",
               (NCAND_TIMES_NH) * sizeof(CP_cand));
    return (RADPUL_EMEM);
  }

  if (uvar.templatebank != NULL) {
    // open template bank
    ...
    while (1) {
      ...
      if (!ferror(templatebank) && (NULL != fgets(line, FN_LENGTH, templatebank)) &&
          !feof(templatebank)) {
    ...
    // Allocate array for template parameters
    template_params = (Binary_Template *)calloc(template_total_amount, sizeof(Binary_Template));
    if (template_params == NULL) {
      logMessage(error, true, "Couldn't allocate %d bytes of memory for template_params.\n",
                 (template_total_amount) * sizeof(Binary_Template));
      return (RADPUL_EMEM);
    }

    // Load templates into an array
    for (i = 0; i < template_total_amount; i++) {
      float P_tmp, tau_tmp, psi_tmp;
      ...
      template_params[i].P = P_tmp;
      template_params[i].tau = tau_tmp;
      template_params[i].Psi0 = psi_tmp;
    }
    ...
    // This is where on-the-fly template bank generation should happen.
    // It should allocate and fill the "template_params" array.
    logMessage(error, true,
               "No template bank file specified (use -t), and on-the-fly template bank generation "
               "not yet implemented. \n");
    return (RADPUL_EIO);
  }
  ...
  if (checkpoint == NULL) {
    ...
    template_counter = 0;
    if (snprintf(cp_head.originalfile, sizeof(cp_head.originalfile), uvar.inputfile) >=
        sizeof(cp_head.originalfile)) {
    ...
    if (fread(&cp_head, sizeof(CP_Header), 1, checkpoint) != 1) {
      ...
      if (cp_head.n_template == template_total_amount) {
        ...
        logMessage(info, true, "Continuing work on %s at template no. %d\n", cp_head.originalfile,
                   cp_head.n_template);
      }
      else if (cp_head.n_template > template_total_amount) {
        logMessage(error, true,
                   "Header checkpoint file %s contains inconsistent information about number of "
                   "templates done (%d > %d).\n",
      ...
      if (strcmp(uvar.inputfile, cp_head.originalfile)) {
        logMessage(error, true,
                   "Input file on command line %s doesn't agree with input file %s from checkpoint "
                   "header.\n",
      ...
      if (fread(candidates_all, sizeof(CP_cand), NCAND_TIMES_NH, checkpoint) != NCAND_TIMES_NH) {
        // XXX checksum for checkpoint
        logMessage(error, true, "Couldn't read all candidates from checkpoint (%s)!",
                   strerror(errno));
        return (RADPUL_EIO);
      }
      ...
      if (uvar.debug) {
        ...
        for (i = 0; i < NCAND_TIMES_NH; i++) {
          logMessage(debug, false, "%d %6.12f %6.12f %6.12f %6.12f %d\n", candidates_all[i].f0,
                     candidates_all[i].power, candidates_all[i].P_b, candidates_all[i].tau,
                     candidates_all[i].Psi, candidates_all[i].n_harm);
        }
      }
  ...
  if (gzread(input, &data_head, sizeof(DD_Header)) != sizeof(DD_Header)) {
    logMessage(error, true, "Premature end of data in file: %s (%s; %s)\n", uvar.inputfile,
               gzerror(input, NULL), strerror(errno));
    gzclose_r(input);
    return (RADPUL_EIO);
  }
  ...
  if (t_series_4bit) {
    ...
    if (t_series_dd_comp4 == NULL) {
      logMessage(
          error, true,
          "Couldn't allocate %d bytes of memory for dedispersed 4-bit compressed time series.\n",
    ...
    if (t_series_dd_comp8 == NULL) {
      logMessage(
          error, true,
          "Couldn't allocate %d bytes of memory for dedispersed 8-bit compressed time series.\n",
  ...
  if ((t_series_4bit &&
       gzread(input, t_series_dd_comp4, sizeof(unsigned char) * n_unpadded_format) !=
           sizeof(unsigned char) * n_unpadded_format) ||
      (!t_series_4bit &&
       gzread(input, t_series_dd_comp8, sizeof(signed char) * n_unpadded_format) !=
           sizeof(signed char) * n_unpadded_format)) {
    logMessage(error, true, "Premature end of data in file: %s (%s; %s)\n", uvar.inputfile,
               gzerror(input, NULL), strerror(errno));
    gzclose_r(input);
    return (RADPUL_EIO);
  ...
  if (t_series_dd.host_ptr == NULL) {
    logMessage(error, true,
               "Couldn't allocate %d bytes of memory for dedispersed uncompressed  time series.\n",
  ...
  logMessage(
      debug, true,
      "Starting whitening of the powerspectrum of the timeseries and zapping of known RFIs.\n");
  if (uvar.white) {
    FILE *zaplist = NULL;
    float norm_factor;
    float *time_series;
    ...
    if (fft_size < uvar.window) {
      logMessage(error, true,
                 "Running median window (%d bins) is too wide for data set (%d bins)!\n",
                 uvar.window, fft_size);
      return (RADPUL_EVAL);
    }

    // use fftwf_malloc for allocation: this is recommended by FFTW (see manual: section 2.1, page
    // 3)
    ...
    // BUT we have to take care explicitly about the zero-padding, as fft wasn't allocated with
    // calloc.
    ...
    for (i = 0; i < n_unpadded; i++) time_series[i] = t_series_dd.host_ptr[i];
    ...
    for (i = 1; i < fft_size; i++) powerspectrum[i] = gsl_pow_2(fft[i][0]) + gsl_pow_2(fft[i][1]);
    ...
    // [0 ................ window_2 ................ white_size - 1 + window_2 ................
    // fft_size - 1]
    ...
    while ((NULL != fgets(line, FN_LENGTH, zaplist) && !feof(zaplist))) {
      ...
      for (idx = idx_min; idx <= idx_max; idx++) {
        // fill FFT bins with Gaussian noise of sigma = sqrt(0.5)*sqrt(uvar.padding) =
        // M_SQRT1_2*sqrt(uvar.padding)
    ...
    logMessage(
        debug, true,
        "Allocating memory for dedispersed,whitenend and zapped uncompressed time series.\n");
    t_series_dd.host_ptr = (float *)calloc(n_unpadded, sizeof(float));

    if (t_series_dd.host_ptr == NULL) {
      logMessage(
          error, true,
          "Couldn't allocate %d bytes of memory for dedispersed uncompressed  time series (2).\n",
          n_unpadded * sizeof(float));
      return (RADPUL_EMEM);
    }
    ...
    for (i = 0; i < n_unpadded; i++) t_series_dd.host_ptr[i] = norm_factor * time_series[i];
  ...
  harmonic_idx_hi =
      (unsigned int)GSL_MIN_INT(fft_size - window_2, (int)(16.0 * uvar.f0 * t_obs + 0.5));
  ...
  while (template_counter < template_total_amount) {
    ...
    P = template_params[template_counter].P;
    tau = template_params[template_counter].tau;
    Psi0 = template_params[template_counter].Psi0;
    ...
    for (harm_idx = 0; harm_idx < 5; harm_idx++) {
      ...
      int last_cand = (harm_idx + 1) * uvar.Ncand - 1;
...
thrA[4] = 0.0;
    ...
    result = run_harmonic_summing(sumspec, dirty, nr_dirty_pages, powerspectrum, window_2,
    ...
    if (binned_spectrum == NULL) {
      logMessage(error, true,
                 "Couldn't allocate %d bytes of memory for downsampled power spectrum.\n",
                 N_BINS_SS * sizeof(unsigned char));
      return (RADPUL_EMEM);
    }

    // check for candidates
    // use dirty page array to find those segments of sumspec arrays that have a chance to
    // contribute a candidate
    for (harm_idx = 0; harm_idx < uvar.harmonics_log2 + 1; harm_idx++) {
      // indices for first and last candidate (for this number of harmonics) in candidates_all[]
      int first_cand = harm_idx * uvar.Ncand;
      int last_cand = (harm_idx + 1) * uvar.Ncand - 1;
      ...
      while (i < fundamental_idx_hi) {
        ...
        for (page_idx = (i >> LOG_PS_PAGE_SIZE);
             (page_idx < nr_dirty_pages) && (dirty[harm_idx][page_idx] == 0);
             page_idx++, i = page_idx * (1 << LOG_PS_PAGE_SIZE)) { /* sic */
          ;
        }
        ...
        // now look for candidates on a "dirty" page of sumspec, but make sure not to go beyond
        // fundamental_idx_hi
        ...
        for (; i < i_next_page; i++) {
          ...
          if (power > thr && power > candidates_all[last_cand].power) {
            ...
            if (store_idx >= first_cand) {
              ...
              qsort(candidates_all + first_cand, uvar.Ncand, sizeof(CP_cand), compare_structs_by_P);
            }
          }

          // if using the sum of 4 harmonics, update screen saver as well
          if (harm_idx == 2) {
            float max_power_screensaver =
                100.0;  // maximum power displayed without truncation in the screensaver
            float powerscale =
                max_power_screensaver /
                255.0;  // power axis scale factor in the computation of downsampled spectrum
            float stepscale =
                (float)N_BINS_SS /
                (float)
                    fundamental_idx_hi;  // conversion factor between full spectrum and SS spectrum
            int bin_ss;

            bin_ss = (int)(stepscale * i);
            if (sumspec[2][i] > powerscale * (float)binned_spectrum[bin_ss])
              binned_spectrum[bin_ss] = (unsigned char)(GSL_MIN_DBL(sumspec[2][i] / powerscale,
                                                                    255.0));  // avoid numbers > 255
          }
    ...
    strncpy((char *)search_params_tmp.power_spectrum, (const char *)binned_spectrum,
            POWERSPECTRUM_BINS);
  ...
  qsort(candidates_all, NCAND_TIMES_NH, sizeof(CP_cand), compare_structs_by_ifa);
...
#define LEN_SHEBANG 40
  ...
  if (!fprintf(output,
               "%% User: %i\n%% Host: %i\n%% Date: %s\n%% Exec: %s\n%% ERP git id: %s\n%% BOINC "
               "rev.: %s\n\n",
               userId, hostId, resultTimeISO, execName ? execName : "unknown",
               erp_git_version ? erp_git_version : "unknown", boinc_rev ? boinc_rev : "unknown")) {
  ...
  while (counter < uvar.Ncand && candidates_all[0].fA > 0.0) {
    ...
    if (fprintf(output, "%6.12f %6.12f %6.12f %6.12f %g %g %d\n", candidates_all[0].f0 * res_factor,
                candidates_all[0].P_b, candidates_all[0].tau, candidates_all[0].Psi,
    ...
    for (j = 0; j < NCAND_TIMES_NH; j++) {
      if (candidates_all[j].f0 == candidates_all[0].f0) candidates_all[j].fA = -10.0;
    }
  ...
  if (rename(uvar.outputfile_tmp, uvar.outputfile)) {
    logMessage(error, true,
               "Couldn't rename temporary output file (%s) to final output file: %s (%s)\n",
  ...
  logMessage(info, true,
             "Statistics: count dirty SumSpec pages %u (not checkpointed), Page Size %d, "
             "fundamental_idx_hi-window_2: %u\n",
...
int set_checkpoint(const User_Variables *const uvar,
  ...
  if (fwrite(candidates, sizeof(CP_cand), NCAND_TIMES_NH_PTR, checkpoint) != NCAND_TIMES_NH_PTR) {
    logMessage(error, true,
               "Couldn't write candidate data to temporary checkpoint file: %s (%s).\n",
               uvar->checkpointfile_tmp, strerror(errno));
    return (RADPUL_EIO);
  }
  ...
  if (rename(uvar->checkpointfile_tmp, uvar->checkpointfile)) {
    logMessage(
        error, true,
        "Couldn't rename temporary checkpoint file (%s) to final checkpoint file: %s (%s).\n",
