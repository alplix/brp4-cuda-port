...
Remember: one driver must support every GPU in the machine - very old and
very new cards cannot share a single driver. Install only ONE package.

Requirements per build
----------------------
  modern  : driver r575+ (Maxwell sm_50 minimum)
  kepler  : driver 470.x/474.x (last Kepler branch), CUDA 10.2-era
  fermi   : driver 390.x/391.x (last Fermi branch), CUDA 8-era

No other dependencies: BOINC, GSL, FFTW, libxml2 and zlib are linked
statically; the two legacy builds carry their own cuFFT library file
(next to the binary), everything else they need comes with the driver.
...
3) Copy ALL files from this package into that folder and make the programs
   executable:
      chmod +x einsteinbinary_BRP4_linux_x86_64_*
...
----------------------------------------------
  ./einsteinbinary_BRP4_linux_x86_64_router \
      -i input.binary -t bank.txt -l zaplist.txt \
      -o results.dat -c checkpoint.dat -W -D 0 -z
(run the era binary directly instead if you do not want auto-routing)
